...
_DELTA_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    # circled 번호(①·⑦·⑬ 등 설계 문서의 결정 항목/형상 번호) — 경계(·) 유무와 무관하게 라인
    # 어디든 나타나면 사설 표식이다.
    ("circled-marker", re.compile(r"[\u2460-\u2473\u3251-\u325f\u32b1-\u32bf]")),
    # `F1`·`F6` 류 사설 fault 라벨 — 앞뒤가 식별자 문자(영숫자·`_`)나 base64 문자(`+`·`/`)가
    # 아닌 `F<숫자>` 단독 토큰만 잡는다 (`MF4`·`F5b` 같은 리뷰-라운드 인용/식별자 일부는 앞/뒤
    # 알파벳으로, `F1_score`·`_F1` 같은 정상 파이썬 식별자는 앞/뒤 `_` 로, base64 임베드 블롭
    # 안의 우연 일치는 앞/뒤 숫자·`+`·`/` 인접으로 제외된다).
