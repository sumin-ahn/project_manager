...
세션 시작 필독 셋은 이미 로드된 진입문서, 현재 정체성의 `pm_state`, `/pm-bootstrap`(claude) / `$pm-bootstrap`(codex) dump 한 번뿐이다.
...
2) 현재 정체성의 pm_state           ← 내 동적 상태(세션 window·남은작업)
   · task: `.project_manager/.local/tasks/<task>/pm_state.md` (세션보다 오래 사는 연속성 앵커)
     신규 task는 `/pm-bootstrap --task <이름>`(claude) / `$pm-bootstrap --task <이름>`(codex) 진입 즉시 생성되므로 호출 전에는 없어도 정상
...
3) `/pm-bootstrap`(claude) / `$pm-bootstrap`(codex) dump (CLI 한 번) — 아래를 한꺼번에 surface:
...
표준 wave: `/pm-bootstrap`(claude) / `$pm-bootstrap`(codex) → 반복{`/pm-wave-claim`(claude) / `$pm-wave-claim`(codex) → `/pm-dev-delegate`(claude) / `$pm-dev-delegate`(codex)(dev/reviewer) → `/pm-wave-finish`(claude) / `$pm-wave-finish`(codex)} → `/pm-handoff`(claude) / `$pm-handoff`(codex). 자세한 구성은 [`pm_playbook.md`](pm_playbook.md) §"Wave 패턴". 호출줄의 실 인자·`--repo <repo> --slot <N>` 값과 전제 경고는 부트스트랩 카드가 단일 진실이다.

| skill | 역할 | 감싸는 내부 엔진 (직접호출 금지) |
|---|---|---|
| `/pm-bootstrap`(claude) / `$pm-bootstrap`(codex) | 세션 시작; board·git·차수·log 본문·남은작업 surface | `pm_bootstrap.py` |
| `/pm-wave-claim T-NNNN`(claude) / `$pm-wave-claim T-NNNN`(codex) | DoD self-containment 검증 + claim | `board.py show/lint/claim` |
| `/pm-dev-delegate T-NNNN --role developer\|code-reviewer`(claude) / `$pm-dev-delegate T-NNNN --role developer\|code-reviewer`(codex) | orchestrator 위임 표준 프롬프트 | `Agent` 툴 |
| `/pm-regression`(claude) / `$pm-regression`(codex) | 비차단 백그라운드 회귀 pre-warm + 완료 알림 | `board.py regression` |
| `/pm-qa`(claude) / `$pm-qa`(codex) | 회귀+lint+git 통합 report | `board.py regression/lint` |
| `/pm-wave-finish T-NNNN`(claude) / `$pm-wave-finish T-NNNN`(codex) | 회귀+log+board+stage; status 미접촉 | `ticket_finish.py` |
| `/pm-handoff`(claude) / `$pm-handoff`(codex) | 세션 종료 7단계 | `pm_handoff.py` |

**무코드/개념(ADR·doc·decision) ticket의 test-less done:** `board.py complete --allow-untested`를 쓰며 본문에 log entry도 없으면 `--allow-missing-log`를 더한다. `/pm-wave-finish`(claude) / `$pm-wave-finish`(codex)(`ticket_finish.py`)도 코드 변경 없는 ticket에는 같은 플래그를 넘긴다.

환경·갱신:

| skill | 역할 | 감싸는 내부 엔진 (직접호출 금지) |
|---|---|---|
| `/pm-env`(claude) / `$pm-env`(codex) | repo/worktree 슬롯·upstream show/switch(path↔URL) | `pm-config.sh`→`pm_config.py` |
| `/pm-update`(claude) / `$pm-update`(codex) | upstream freshness 자동분기·manifest reconcile·adapter-drift 표면화 | `pm-update.sh`→`pm_update.py` |
...
1. `/pm-update`(claude) / `$pm-update`(codex)로 prefix 도구를 흡수.
...
[`pm_playbook.md`](pm_playbook.md)는 해당 활동 때만 읽는다: 위임 시 "위임 — 두 가지 방식", wave 시 "Wave 패턴"(9단계+메타 학습), 운영 시 "PM 운영 효율 규칙", ticket 발행·분할 시 "메타 정책", 핸드오프 시 "다음 PM 부트스트랩 프롬프트 템플릿"(`/pm-handoff`(claude) / `$pm-handoff`(codex) 자동 추출).
...
`/pm-handoff`(claude) / `$pm-handoff`(codex) skill(backbone `pm_handoff.py`)을 사용하고 dry-run을 권장한다(`--dry-run`). task 경로는 `/pm-handoff --task <이름>`(claude) / `$pm-handoff --task <이름>`(codex)만 쓰며 backbone도 `pm_handoff.py --task <이름>`으로 차수·기본 요약·보유 작업공간을 해소한다. 다음 트리거는 `/pm-bootstrap --task <이름>`(claude) / `$pm-bootstrap --task <이름>`(codex)이다.
...
5. `pm_playbook.md` §"다음 PM 세션 부트스트랩 프롬프트 (템플릿)"의 역할 framing+`/pm-bootstrap`(claude) / `$pm-bootstrap`(codex) 트리거를 stdout 출력. 인계 본문은 채우지 않으며 log entry가 단일 진실이고 다음 bootstrap이 차수·인계 본문·남은작업을 dump.
...
- 마지막 응답에 인계 트리거를 코드블록으로 출력한다. 다음 세션은 `/pm-bootstrap`(claude) / `$pm-bootstrap`(codex) 실행; 인계 본문은 bootstrap이 log entry에서 dump한다.
...
[`pm_playbook.md`](pm_playbook.md) §"다음 PM 세션 부트스트랩 프롬프트 (템플릿)"을 `/pm-handoff`(claude) / `$pm-handoff`(codex)(backbone `pm_handoff.py`)가 자동 추출해 stdout 출력한다.
