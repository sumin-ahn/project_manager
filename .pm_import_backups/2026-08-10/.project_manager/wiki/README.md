...
이 프로젝트의 모든 비-코드 산출물 (작업·결정·사양·상태·계획)이
...
├── pm_state.md       # PM 동적 상태 (세션 window / 진행 중 의사결정 / 남은 작업) — `/pm-handoff`(claude) / `$pm-handoff`(codex) 가 갱신
...
| `pm_role.md` / `pm_state.md` / `pm_playbook.md` | PM 세션 인계 3분할 — **정적 핵심**(role·매 부트스트랩 로드) / **동적 상태**(state·세션 window 등, `/pm-handoff`(claude) / `$pm-handoff`(codex) 자동 갱신) / **활동 레퍼런스**(playbook·위임·Wave·메타정책·인계 템플릿, 해당 활동 시 lazy Read). `pm_handoff.py` 가 인계 템플릿을 playbook 에서 추출 |
