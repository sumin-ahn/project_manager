...
핵심 인계 사항 (lean handoff — §handoff 철학 · 대량 재열거 금지):
<- 읽기 범위: 이 entry + 인용할 과거 entry/ADR. (board/git/log 는 /pm-bootstrap 가 라이브로 답함 — 적지 마라.)
   메타 학습: ticket 상태에서 도출 불가한 교훈만. 없으면 "없음" (필드는 유지).
   다음 intent (두 줄 세분 — ADR-0008·T-0047):
     - 대화 thread-tail: 정지 직전 사용자 발화 (ctx-trigger 경로는 어댑터 자동 — 초안이라 정지 후 PM 이 검토·편집).
     - pending user intent: 다음 우선순위 + 사용자 결정 대기.
   회귀/incident: 회귀 "N passed / 상태" 1줄 (green 도 — baseline) + 비-자명 incident.>

지금 부트스트랩 시작하세요.
```

> **정지 후 PM 손 (ctx-trigger 경로):** 어댑터가 채운 *대화 thread-tail* 슬롯은 자동 추출 **초안**이다 —
> 새 PM 세션은 그 슬롯을 검토하고 필요시 편집한다(민감 발화·과/부족 추출 보정). *pending user intent* 는 항상 PM 손.
