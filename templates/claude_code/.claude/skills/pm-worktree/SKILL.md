...
"이 submodule 은 내가 작업 중"·"submodule drift 정리해" 라고 지시하면 PM 이 이 스킬을 부른다.
셋업(pm-env·user-entrypoint)의 확장이 아니라 **운영중-관리** 스킬 — 청중이 다르다(ADR-0049).
...
- 스킬은 backbone 호출만 하는 얇은 래퍼 — `dev`/`sync` 의 stdout(무엇을 했는지·skip 사유·경고)을
  PM 이 읽고 사용자에게 보고한다.
