...
1. **read-only** — 파일을 만들거나 고치지 않는다. 산출은 보고뿐이며, 대형 보고는 아래 산출 아티팩트 예외를 따른다.
...
3. 지정된 파일/디렉터리/레퍼런스/로그를 `grep`/`glob`/`Read`/`Bash`(읽기 전용)로 조사.
...
보고가 대략 200줄/8KB를 넘길 것 같으면 인용·목록을 작업 디렉터리 안 파일(이름에 ticket·주제 명시·Bash로 기록 — 조사 대상은 건드리지 않고 산출 아티팩트만 생성)에 쓰고, 응답에는 **절대경로 + 핵심 요약 ≤10줄**만 반환한다. 출력 상한의 조용한 절단을 피하기 위해 큰 수집물은 파일로 전달한다.
...
- 파일 수정·생성(read-only). 단 위 대형 보고 산출 아티팩트 예외.
