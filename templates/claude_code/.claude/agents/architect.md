...
실측·불변식·표면 상한·테스트 전략과 구현 가능한 인터페이스 판단을 사실 중심으로 쓴다. 첫 줄 헤더는
그대로 두고 그 아래 시드된 골격을 채운다. 같은 디렉터리의 `spec.md`(티켓 명세)와 `rounds/`(이전
라운드)는 읽기 전용 입력이며 PM 홈 티켓은 수정하지 않는다. 리뷰가 설계 결함으로 판정한 재위임이면
`rounds/`의 이전 설계·구현 보충·리뷰를 대조해 이번에 준비된 라운드 파일에 결함과 바뀐 결정을
기록한다. 파일 이름·순번은 엔진이 만들고, 자기평가·장황한 서사는 금지하며 결정·발행 권한은 계속
