...
티어는 **`--tier hard`**(cross·pm_delegate) 또는 **native 경로의 hard 프로필**(codex
`developer-hard` agent 등)로 선택한다. 티어 매핑은 **하네스-중립** — 어느 하네스든 normal/hard
...
해소된 **target harness == 내(PM) 하네스**면 → **네이티브 위임**을 쓴다(아래 §실행 패턴 그대로 —
