...
> {{PROJECT_NAME}} 릴리즈 수동 체인(adopter#0 sync 선행 → livegate record → main push → tag →
> GitHub Release 생성/완결 확인 → adopter#0 흡수)을 **순서 고정** 절차로 codify 한다. 비즈니스 로직 0 —
> 기존 엔진 CLI(`board.py livegate record/check` · `pm_update`) + `git`/`gh` 호출을 얇게 감싼다
> (ADR-0049 명령어化 4요소). **공개 main push 는 사용자 승인 게이트를 유지** — 이 커맨드는 절차를
> 자동화하되 **push 를 대신 실행하지 않는다**(보호훅 + livegate 기계 이중 안전 불변).
...
**pm-internal** — PM 에이전트가 릴리즈를 낼 때 invoke. 사용자가 "vX.Y.Z 릴리즈 내자"·"태그 찍고
GitHub Release 만들어" 라고 지시하면 PM 이 이 커맨드를 부른다. 셋업(user-entrypoint)의 확장이 아니라
**운영중-관리** 커맨드다(ADR-0049).

## 사용 시점 (trigger)

- 보호 브랜치(`main`)로 릴리즈를 낼 때 — CHANGELOG 절 확정 후 push→tag→GitHub Release 낼 준비가 됐을 때.
- v1.3.0 자신이 이 커맨드의 첫 도그푸딩 소비자다(PM 71 의 3연속 patch 수동 릴리즈 고통이 근거).

## 실행 (순서 고정 · ADR-0039)

opencode bash tool 로 실행한다. `$ARGUMENTS`(릴리즈 버전·repo)를 추출해 아래 순서를 지키되, **push
순간은 사용자 승인을 기다린다**. 엔진이 인코딩을 코드로 처리하므로 env prefix 불필요(드물게 필요하면
셸별 문법 — bash `PYTHONUTF8=1`, PowerShell `$env:PYTHONUTF8='1';`).

### 1. adopter#0 sync 선행 (false-green/false-block 차단)
...
건너뛰지 않는다(기계 단계). 상세 분기(freshness·manifest reconcile·drift)는 pm-update 커맨드 절차를 따른다.
...
1. **main push — 사용자 승인 게이트(이 커맨드가 자동 실행하지 않는다)**. 보호훅이 `livegate check`
   green(§2 기록)을 추가로 요구한다. push 는 **사용자 승인 후** 실행한다:
   ```bash
   # 사용자 승인 필요 — 커맨드는 이 명령을 대신 돌리지 않는다(보호훅 + livegate 이중 안전).
...
- **공개 main push = 대화 승인 유지(불변)**: 커맨드가 절차를 자동화하되 push 순간은 사용자 승인 게이트
  (보호훅 T-0076 + livegate 기계)의 **이중 안전을 깨지 않는다** — 커맨드가 자동 push 하지 않는다
  (② private 만 자율·① public main 은 승인). `PM_ALLOW_PROTECTED_PUSH=1`·`PM_SKIP_LIVE_GATE=1` 은
  스스로 쓰지 않는다(사용자 명시 OK 의 escape hatch·환경 문제는 우회 사유 아님).
...
- 커맨드는 backbone 호출 + 승인 대기 절차 — 각 단계 stdout(livegate 수집 N·gh release view 결과)을
  읽고 사용자에게 보고한다.
- **main push 실행·CHANGELOG 절 문안·GitHub Release 노트**는 PM/사용자가 확정·승인한다(커맨드 자동 아님).
