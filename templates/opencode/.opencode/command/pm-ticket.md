...
- `--design`: 설계 단계 상태. 값은 `required` / `done` / `n/a` 세 형식이며(references 설계 단계 표), 인식 불가한 값은 발행 전에 거부된다. **면제 경로는 없다** — 설계를 건너뛰는 값을 두면 그 티켓만 라운드 순번이 어긋나고 면제가 곧 우회 플래그가 된다. 생략 시 `--estimate large`면 `required`, 그 외는 `n/a`로 박힌다. 설계 검증이 필요하다고 판단하면 small/medium에도 `--design required`로 명시 지정한다.
- 출력의 `created <T-NNNN>`로 ID를 확인한다.
- 발행은 티켓을 **운영 단위(묶음)에 귀속**시키고 그 사실을 stderr 1줄(`클러스터 귀속: <C-이름>`)로 고지한다. 선언된 활성 묶음이 정확히 하나면 거기에 넣고, 없으면 그 티켓 이름의 **크기 1 장부**를 함께 만든다(`(크기 1 장부 생성)` 표기). 활성 묶음이 둘 이상이면 엔진이 고르지 않고 크기 1로 발행하며, 나중에 `board.py cluster new` 가 흡수한다.
...
- **## 설계**: `design: required`인 티켓만 채운다(references 설계 단계 절). `n/a`면 뼈대를 그대로 둬도 게이트 비대상이다.
...
## 3. architect 점검 라운드 (묶음 1회 · 초안 → 점검 → 비준)
...
점검도 **묶음 1회**다 — 세션 하나가 run-dir 하나에 깔린 티켓별 라운드 파일 N개를 채운다. 티켓
하나짜리 묶음도 같은 경로다(특례 없음).
...
- `promote`는 승격 전 본문을 재검사한다. placeholder(5절 뼈대 문장·`xxxxx`·`T-XXXX` 등)나 누락 표준 절이 있으면 **거부**(rc=1)하고 이슈를 나열한다. 설계 절 뼈대는 이 5절 집합과 **별도 집합**이라 `design: required|done` 티켓에만 적용된다 — `n/a`는 뼈대가 남아 있어도 통과한다. draft는 `.drafts/`에 남으므로 더 채운 뒤 재시도한다.
- `design: required`는 설계 절을 다 채웠어도 promote가 거부한다. **설계 검토 완료(`design: done`)가 open 진입 조건**이기 때문이다.
