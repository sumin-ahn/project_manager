...
> 후행 "차" 정규화로 이중부착 방지·T-0100). 차수는 `--session-seq` 로만 준다(구형 차수 별칭은
> ADR-0057 로 제거). multi-PM 이면 세션 정체성은 canonical `--repo <repo> --slot <N>` 으로 준다
> (구형 세션·worktree-슬롯 별칭은 ADR-0057 로 제거).
