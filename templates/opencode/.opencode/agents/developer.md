...
{{PROJECT_CONSTRAINTS}}
<!-- TODO: AGENTS.md "프로젝트 고유 제약" 절과 동일한 내용을 여기 박는다.
     subagent 는 fresh 컨텍스트로 시작하므로 이 제약이 정의에 직접 들어 있어야 한다.
     예시 (도메인 무관):
...
subagent 도 프로젝트의 PM 사용자 게이트·금지 항목을 그대로 상속한다 (`.project_manager/wiki/pm_role.md` §"금지 (PM·사용자 단독 불가)"·§"사용자 게이트", AGENTS.md §5). 외부 비가역 행위·미션 변경·{{PROTECTED_PATHS}} 수정 등은 이 subagent 의 권한 밖이다.
