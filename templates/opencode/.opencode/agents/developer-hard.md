...
> **대형 산출물은 파일로 — 응답(보고) 절단 우회.** 위 보고가 대략 200줄/8KB 를 넘길 것 같으면 본문을 작업 디렉터리 안 파일(이름에 티켓·주제 명시)로 쓰고, 응답엔 그 **절대경로 + 핵심 요약 ≤10줄**만 반환하라. opencode 에선 대형 파일 쓰기에 safe_write(8KB 청크)를 쓴다 — write 는 16KB 초과를 거부한다. 응답 채널은 출력 상한(약 32K 토큰)에서 조용히 잘리고 수신자(PM)는 절단을 알 수 없으므로, 큰 산출은 읽기 채널(파일)로 넘겨야 온전히 전달된다.
