...
function safeWrite(root, filePath, content, mode, chunkBytes) {
  ...
  // ── 경계 접착 방지 강제 게이트(line-aligned 계약 · T-opencode-002 라이브 실측) ──────────────
  ...
  if (mode === "append" && exists) {
    ...
    if (prev.length > 0 && !prev.endsWith("\n") && !content.startsWith("\n")) {
      throw new Error(
        "[safe-write] 대상 파일이 개행(\n) 없이 끝난다 — 이 상태로 append 하면 앞 조각과 " +
          "붙어 손상된다. 이번 조각의 첫 문자를 \n 으로 시작하라(또는 의도한 mid-line " +
          "이어쓰기가 아니면 먼저 read 로 파일 끝을 확인하라).",
      );
    }
  }
