#!/usr/bin/env python3
"""추가 리뷰어 래퍼 — 추가 리뷰어(하네스) 어댑터 CLI.

사람 역할 이름은 **추가 리뷰어(additional reviewer)** 다 — 팀에 한 명 더 붙는 리뷰어다.
설정 키는 `additional_reviewer.*`(대상 tuple·상한)이고, 모듈·신규 raw
...
채널 스위치는 없다:
  - 추가 리뷰어는 developer·architect 와 같이 **부르면 도는 역할**이다. 켜고 끄는 설정 키도, 실행
    전 동의 확인도 없다 — 저장소 내용을 모델 API 로 보내는 것은 PM 이 파일을 읽는 순간 이미
    일어나는 이 프레임워크의 정상 동작이라 특별 취급할 근거가 없다. 호출 횟수 상한(라운드·wave
    예산)만 남는다. 단 빈/공백 diff 는 dry-run 포함 무조건 exit 1 (false-green 원천 차단).
...
종료 코드/신호:
  ...
  - 게이트 스냅샷 또는 미등록 linked worktree 자기 앵커 + 게이트 라운드 → exit 1 (실행 전
    ...
    스냅샷 마커가 있거나 PM 홈을 해소하지 못한 linked worktree에서는 실 호출 라운드를
...
설계:
  ...
  - 구조화 리뷰어 호출의 권한은 code-reviewer read 축으로 고정하며 설정으로 쓰기 권한을 올릴 수 없다.
  - 추가 리뷰어는 위임과 **같은 실행 조건**을 받는다: cwd 는 검토 대상 저장소(diff_root)이고 env 는
    위임 채널과 같은 seam(`pm_delegate.build_env`)이 조립한다. 채널이 다르다고 다른 권한·다른 홈을
    주지 않는다. 회신 본문의 품질 검사(`detect_output_contamination`)는 그대로 남는다 — 엇갈린 판정·
    옛 raw 인용이 섞이면 이번 판정이 이 리뷰의 것이라는 보장이 없어 '판정 불명확'으로 접는다.
  - 라운드 상한 기계 차단: 추가 리뷰어 호출(유료)이 무한 반복되지 않게 `--gate <T-NNNN>`
    별 라운드 장부(`.project_manager/.local/review_rounds.json`·per-clone·git-ignored)에 실 호출을
    count 하고, 수렴 축(must_fix 추이)과 미완 축이 상한에 닿으면 실행 *전에* 거부(exit 4)한다.
    호출 횟수만 세던 판정 라운드 상한은 제거됐다 — 수렴 판정이 같은 범위를 더 정확히 본다.
    "몇 라운드나 돌았나"라는
  ...
  - 라운드별 산출 장부 + wave 예산: 같은 장부에 라운드마다 산출(`rounds` — 판정 rc·must-fix 수)을
    append 하고, 전 게이트 합계 호출을 wave 단위로 센다(`wave` 절). 라운드 수만으로는 "그 라운드가
    실결함을 냈는가"를 기계로 확인할 수 없어 게이트 심도 대비 비용 적정성 판단이 PM 자기보고에
    의존했다. 조회는 `--rounds-report`(호출 없는 읽기 전용 표). 기록은 무조건이고 hard 거부는
    예산 축 하나뿐이다.
"""
...
_ENGINE_REV_SKEW_RECOVERY_REASONS = {
    ...
    "shared_read_seam": (
        "판독은 형제 없이도 떠야 한다 — 이 모듈의 진단·재앵커 경로는 seam 이 필요한 "
...
def _scope_from_initial_pm_home(*, ticket_selected: bool, explicit_paths: bool) -> bool:
    ...
    (`DEFAULT_PATHS`)로 떨어졌든 **범위의 출처는 최초 PM 홈 하나**라, 표시 conf 와 실제 호출
...
# 미완 라운드 상한 — 판정 없이 끝난 호출(타임아웃·중단·오염 무효화)이 같은 게이트에서 이 횟수를
# 채우면 이후 실행을 거부한다. 호출 횟수만 세던 판정 라운드 상한은 같은 범위를 보는 수렴 축과
...
# 이 축은 호출 횟수가 아니라 **장부의 must_fix 추이**로 수렴 여부를
...
# 티켓 수 × 라운드 상한으로 확장되므로, 전 게이트 합계 호출을 이 예산으로 묶는다. 기본 24 는
...
# 실행 전 거부라 리뷰어는 호출되지 않는다(과금 없음). wave 예산 소진도 같은 축(호출 전
...
# 미완 라운드 상한 초과 fail-loud 안내. 같은 게이트에서 판정 없이 끝난 호출이 상한을 채우면
...
_ROUND_LIMIT_GUIDANCE = (
    ...
    "  · **미완**은 판정이 없던 호출입니다 — 타임아웃·중단뿐 아니라 **오염 진단으로 무효화된 "
...
# 수렴-형상 차단 안내 (라운드 상한 rc 를 그대로 쓴다 — 호출 전 예산 거부라 같은 축).
...
# `--ticket` 만 준 실행은 리뷰를 정상 호출·과금하면서 라운드 예약·기록·상한 회계를 통째로
...
# 깨지 않게 provenance 직후에 낸다. 빈 diff·diff-cap 으로 아직 끝날 수 있는 예약 전 자리이므로,
# 장부에 붙었다는 확정형이 아니라 실행 조건형으로만 말한다.
_GATE_DERIVED_NOTICE = (
    "게이트 자동 유도: --gate {gate} (--ticket 값에서 유도 — 이 실행이 리뷰어를 부르면 라운드 회계가 "
    "게이트 {gate}에 붙습니다). "
...
_GATE_DERIVED_DRY_RUN_NOTICE = (
    ...
    "않고, 실 호출 때 라운드 회계가 이 게이트에 붙습니다)."
)

# 회계 밖 실행의 loud 표기 — 침묵이 이 함정의 본체였으므로 명시 opt-out 실행은 자기
# 상태를 스스로 말한다. 아무 선택도 없는 실 호출은 아래 `_GATE_ACCOUNTING_REQUIRED_GUIDANCE`로
# 거부하므로, 이 조건형 경고는 `--no-gate` 경로에서만 나온다.
# 예약 자리(호출 확정 전)의 문구는 **조건형**이다: 이 뒤로 환경 준비·스폰이 남아 있어 "호출했다"고
# 확정하면 스폰 전에 중단된 실행이 이미 찍힌 고지와 모순된다. 확정 표기는 실행이 끝난 뒤 판정
# 블록(`print_summary`)의 게이트 줄이 낸다.
_UNACCOUNTED_RUN_TAIL = (
    "리뷰어를 부르면 라운드 장부에 기록되지 않고 라운드·wave 예산도 쓰지 않습니다 — 그 라운드에서 나온 "
...
# 실 실행은 회계에 넣거나 명시적으로 빠져야 한다. 어느 쪽도 고르지 않은 실행을 경고만 하고
# 진행하면 티켓 누락 한 번이 라운드 장부·livegate 누락으로 이어진다. dry-run·조회·처분과
# diff-cap 조기 종료는 리뷰어를 부르지 않아 이 검사에 도달하지 않는다.
_GATE_ACCOUNTING_REQUIRED_GUIDANCE = (
    "오류: 실 호출에는 게이트 회계 지정 또는 명시 opt-out 이 필요합니다 — "
    "이 실행은 호출하지 않았습니다.\n"
...
# livegate 기록이 조용히 초기화된다. 명시 `--paths` + `--no-gate`는 장부 없는 raw 호출 복구
# 채널로 기존대로 열어 두되, **장부 기록이 생기는 실 호출**만 예약 직전 fail-loud 한다.
_GATE_SNAPSHOT_ROUND_GUIDANCE = (
    "오류: 게이트 스냅샷 마커가 있는 앵커에서는 실 호출 라운드를 기록할 수 없습니다 — "
...
_RESOLVE_GATE_MODE_GUIDANCE = (
    "오류: `--resolve-gate {gate}` 에는 `--pm-verified`가 필요합니다 — 이 실행은 아무것도 "
    "하지 않았습니다.\n"
    "  · PM 판정 표면의 기계 확인 증거로만 해소합니다(재호출 없음 — accepted 잔여 "
    "0·잔여 must-fix 건수만큼 전건 처분·필요 시 기계 확인 1건 이상).\n"
    "  · 후속 티켓·후속 게이트로 잔여를 넘기는 처분은 지원하지 않습니다."
)
...
def knob_value_key(conf: dict[str, str], key: str) -> str | None:
    ...
    빈 값은 종전부터 미설정으로 읽어 기본값으로 fail-soft 했다. 그 의미를 그대로 승계해
    "비어 있지 않은 값"만 공급으로 본다.
...
# 우리가 띄우는 행위자를 `외부` 로 부르지 않는다 — 설정 키는 대상·상한 `additional_reviewer.*`
# 뿐이고, raw 파일 접두·모듈 파일 이름 같은 **이미 기록된 산출물에 박힌 기계 식별자만 그대로**
# 유지한다.
#
# 대상은 위임과 동형의 **원자 tuple**(harness+model 동반 필수·reasoning 선택) 하나뿐이다.
# 모델을 고정하지 않는 자유 문자열 경로는 "어느 모델이 봤는지"를 사후에 알 수 없어 라운드 장부·
# raw 감사가 거짓말을 하게 되므로 폐지했다 — tuple 이 없으면 호출하지 않고 fail-loud 다.
...
# 실제 cwd/프롬프트 파일이 들어간다(opencode 만 해당).
REVIEWER_CWD_PLACEHOLDER = "<review-cwd>"
REVIEWER_PROMPT_FILE_PLACEHOLDER = "<prompt-file>"


class ReviewerTargetError(RuntimeError):
    """추가 리뷰어 대상 설정이 원자 tuple 계약을 어김 — 호출 전에 중단해야 함."""
...
def _structured_reviewer_argv(
    ...
    모델·reasoning 은 **항상 argv 에 명시**되고 권한축은 code-reviewer(읽기)로 고정된다. 사용자
    config 기본값(모델·effort)이 실제 수신자를 바꿀 수 없는 이유가 이것이다 — 명시 플래그가
    config 기본값을 이긴다.

    codex/claude 는 프로세스 cwd 를 그대로 쓰므로 실행마다 달라지는 토큰이 argv 에
...
def resolve_reviewer_target(conf: dict[str, str]) -> ReviewerTarget:
    ...
    모든 거부는 **호출·라운드 예약·raw 예약 전**에 성립한다 — 호출부가 이 함수를 그 게이트들
...
# ── 라운드 상한 장부 ─────────────
# 추가 리뷰어 호출은 유료 호출이라 라운드가 무한정 이어지면 비용이 쌓인다(PM 10차 실측: 한
# 게이트 클러스터 25라운드). PM 자의 라운드 집계를 기계 장부로 대체한다
# ([[mechanize-dont-instruct-llm]]): `--gate <T-NNNN>` 별로 실 호출 횟수(count)와 라운드별 산출을
...
def _load_file_lock():
    ...
    로드한다 — 이 seam 이 필요한 구간은 `--gate` 실행의 라운드 장부 예약/마감과 호출 산출물의
    접근 제한뿐이라 나머지 경로(진단·재앵커를 deep-import 로 재사용하는
...
# 기능 축 그대로 **형제 없이도 떠야 한다**(진단·재앵커는 seam 이 필요한 `--gate` 구간
...
def _restrict_to_owner(path: Path) -> None:
    """호출 산출물을 소유자 전용 접근으로 제한한다 (플랫폼 수단은 공용 seam 소유).

    `os.chmod(0o600/0o700)` 을 직접 부르지 않는 이유는 그 호출이 **Windows 에서 아무 제한도
    걸지 않기 때문**이다(실측 `S_IMODE`=0o666). opencode transport 샌드박스에는 검토 대상 diff
    원문이 들어가므로 "다른 사용자에게 읽히지 않는다" 는 이 도구의 보안 경계다 —
...
def _target_rev_fingerprint(diff: str) -> str:
    """리뷰어에게 실제 넘길 diff의 content revision (`sha256:<hex>`).
...
def _reserved_gate_error(gate: str | None, *, flag: str = "--gate") -> str | None:
    ...
    판정 입력은 이름 하나뿐이라 호출·장부 접근 **전에** 부를 수 있다 — 거부된 실행은 호출도
    장부 변경도 남기지 않는다. 게이트 이름을 받는 표면이 여럿이라(`--gate`·`--resolve-gate`)
    어느 플래그가 걸렸는지는 인자로 받는다 — 판정 규칙은 한 곳이다."""
...
def _recorded_round_outcomes(ledger: dict, *, since: str | None = None) -> int:
    ...
    손상된 `spent` 를 0 으로 접지 않기 위한 **데이터 근거 재계산**이다. 산출은 실제 호출된 라운드
...
def _spend_wave_round(state: dict) -> None:
    """실 호출 1회를 wave 예산에서 쓴다 — 첫 호출이 wave 시작 시각을 찍는다.
...
def _refund_wave_round(state: dict, wave_id: str | None) -> bool:
    ...
    소비가 0 으로 돌아가면 시작 시각도 지운다 — `started` 는 **첫 실 호출** 시각이라, 호출이 하나도
    없는 wave 에 시각만 남으면 조회 표가 있지도 않은 wave 를 진행 중으로 보여주고 다음 첫 호출이
...
def _reset_wave(ledger: dict) -> dict:
    """`--ack-wave` — ack 시점에 예산을 리셋한다(다음 호출이 새 시작 시각을 찍는다).
...
def _gate_entry(ledger: dict, gate: str) -> dict:
    """게이트 항목을 호출 레코드를 포함한 현 스키마로 정규화한다.
...
def _round_counts(entry: dict) -> tuple[int, int, int]:
    """이 게이트의 (전체, 판정, 미완) 호출 수; 구 장부 count는 판정으로 보수 승계한다."""
    ...
    # 레코드가 없던 시절의 호출(count 만 있는 구세대 항목)은 판정으로 세는 쪽이 보수적이다.
...
def _reserve_round(
    ...
    """단조 sequence identity로 호출을 예약하고 레코드를 반환한다.
...
def _inflight_reservations(entry: dict, *, wall_timeout_sec: int | None = None) -> int:
    ...
    2완료 + 2예약 동시 통과 → 4호출). 예약은 장부 임계 구역 안에서만 만들어지므로, 그 구역에서
    보는 미마감 예약을 상한에 더하면 창이 닫힌다.

    **미완 재시도 상한(`additional_reviewer.incomplete_rounds_max`)과 역할이 다르다.** 그쪽은
    "판정을 못 낸 호출을 몇 번까지 다시 시도하나"(호출 횟수 축)이고, 이 수는
    "지금 몇 라운드가 이미 나가 있나"(수렴 축의 동시성)다. 같은 레코드를 보지만 묻는 질문이
    달라 한쪽이 다른 쪽을 대신하지 못한다 — 미완 상한(기본 2)은 상한 3 설정을 넘기는 4호출 창을
    막지 못하고, 이 수는 재시도 예산을 세지 않는다.

    호출이 확실히 없던 예약은 환불로 레코드 자체가 사라지므로(`_refund_round`) 여기 남지 않는다.
...
# ── 예산 게이트: 확인→예약 (호출보다 **앞**) ────────────────────────────────
# 두 예산(게이트 라운드 상한·wave)의 확인과 예약은 raw 선점·스폰보다 먼저다. 이미
# 상한에 닿은 호출이 raw 자리와 산출 디렉터리를 만들었다 지우면 "차단된 호출은 아무것도 하지
# 않는다"가 거짓이 된다 — 정리가 끝까지 성공한다는 보장도 없어(정리 실패는 loud 경고로 남는다)
# 호출도 못 할 실행이 산출 자리를 남길 수 있다.
#
# 순서를 뒤집는 대가는 하나뿐이다: 예약 뒤·스폰 전에 끝나는 구간(리뷰어 환경 준비)이
# 생긴다. 그 구간은 호출이 확실히 없으므로 마감 시점의 `started=False` 환불과 **같은 조건**
...
class RoundBudget(NamedTuple):
    ...
    `refused_rc` 가 채워진 결과는 **그 자리에서 끝나는 실행**이다(raw·스폰·과금 없음).
...
def _derive_gate_from_ticket(args) -> GateDerivation:
    ...
    않은 선택으로 둔갑한다(그 두 면은 호출도 예약도 하지 않아 여기서 닫을 구멍이 없다).
    ...
    # 호출·예약하지 않아 `--gate`는 필터/무시 입력, `--no-gate`는 무시 경고 대상이다.
...
def _reserve_round_budget(
    ...
    """라운드 상한·wave 예산을 한 임계 구역에서 확인하고 이번 호출을 예약한다.

    여기까지 왔으면 dry-run·빈-diff 거부를 모두 통과해 *실 호출*이 일어난다 — 그것들은 리뷰어를
    부르지 않아 라운드가 아니므로(카운트 제외) 이 앞의 조기 return 뒤에
    게이트를 둔다. 게이트가 해소된 실행에서만 per-gate 장부를 대조한다(게이트 없는 실행 = 상한
    대상 밖).

    게이트가 있는지는 이 함수보다 앞에서 정해진다 — `--ticket` 실행은 `_derive_gate_from_ticket`
    이 게이트를 유도하고, selector 에 티켓이 없으면 실 호출 seam 이 명시 `--no-gate`를
    ...
    호출·과금됐을 수 있는데 성공시에만 세면 반복 타임아웃으로 상한을 무한 우회한다. 자식 프로세스가
    ...
    if not args.gate:
        ...
        if args.ack_wave:
            print("경고: --ack-wave 는 --gate 와 함께 써야 합니다 (게이트 단위 장부) — 무시.",
                  file=sys.stderr)
        # 호출이 확정된 구간이지만 **아직 스폰 전**이다(환경 준비가 남아 있다) — 그래서 문구는
        # 조건형이고("이 실행이 호출되면"), 확정 표기는 실행이 끝난 뒤 판정 블록의 게이트 줄이
        # 낸다. 그래도 여기서 한 번 말하는 이유는 침묵이 이 함정의 본체였기 때문이다(장부 0건·
        # 릴리즈 차단 미도달) — 호출 전에 멈출 기회를 PM 에게 준다.
    ...
    try:
        with _round_ledger_lock():
            ...
            # (2) 미완 라운드 상한 (판정 없이 끝난 호출 축). 호출 횟수만 세던 판정 상한은
            ...
            if incomplete >= incomplete_limit:
                ...
                # 예약 없음 (호출 전 거부) — raw 자리도 아직 없다(이 게이트가 그보다 앞이다).
                return RoundBudget(refused_rc=EXIT_ROUND_LIMIT_EXCEEDED)
            ...
            _spend_wave_round(wave)                   # 같은 호출을 wave 예산에서도 차감
            _save_round_ledger(ledger)
            return RoundBudget(
        ...
        # 락 획득/장부 write 실패 — 상한을 확인하지 못한 채 호출하면 과금 게이트가 무력화되므로
        # **호출 전에** 멈춘다(과금 0). 가장 흔한 원인은 동시 실행의 락 보유다(Windows
...
def _release_round_reservation(reservation: RoundBudget, *, reason: str) -> bool:
    """호출 **전에** 끝난 실행의 예약을 그 자리에서 원자 환불한다 (락→로드→환불→저장).
    ...
    try:
        ...
        print(
            f"경고: 라운드 예약 환불 실패 ({type(exc).__name__}: {exc}) — {reason}(으)로 호출 "
    ...
    if refunded:
        print(
            f"라운드 예약 환불: 게이트 {reservation.gate} — {reason}(으)로 호출 없이 "
...
class _PreSpawnReservation:
    ...
    예산 게이트가 앞에 서면서 "예약은 잡혔는데 리뷰어는 아직 못 떴다"는 구간이 생겼다
    (리뷰어 환경/커맨드 준비). 이 구간에서 어떤 이유로 빠져나가든 호출은
    확실히 없으므로 예약은 되돌아가야 한다. 환불 조건을 예외 **종류**로 잡으면 등재되지 않은
    예외가 예약을 finished_at 없는 미완 레코드로 남겨, 호출도
    ...
    환불은 하되 예외는 그대로 다시 던진다 — 예상 못 한 실패를 판정 rc 로 바꾸면 진단이 사라진다.
    ...
    `_watchdog_reviewer_run` → relay 워치독의 `on_spawn_attempt`). 그 뒤의 실패는 호출이 이미
    ...
    가 실측 축이다) 예약이 finished_at 없는 미완 레코드로 남아, 호출 0·과금 0 인 실행이
...
def render_rounds_report(
    ...
    *,
    ...
    lines = [
        ...
        "처분: 미처분=잔여 must-fix 선언 없음(릴리즈 차단) · "
        "pm-verified=현재 티켓 fix의 기계 확인 완료 · 무대상=잔여 없음",
    ]
    if include_wave:
        lines.insert(
            1,
    ...
    for name in names:
        ...
        if not rounds:
            lines.append("  (라운드 산출 기록 없음 — 산출 장부 이전의 호출)")
            continue
...
def _print_rounds_report(
    ...
    try:
        ...
        # 조회는 호출이 없어 conf 판독 실패로 막을 이유가 없다(호출 경로의 fail-closed 와
...
# 라운드 상한으로 종결된 게이트의 잔여 must-fix가 현재 티켓의 fix에서 기계적으로
# 닫혔음을 장부에 남기는 기록면이다(호출 없음). 후속 티켓·후속 게이트로 넘기는
# 처분은 고정 5단계 수렴 계약과 충돌하므로 제공하지 않는다.


def _describe_resolution(declared: dict) -> str:
    """현재 유일한 처분 선언을 사람용 한 줄로 표기한다."""
...
def _resolve_gate_ignored_flags(args: argparse.Namespace) -> str:
    ...
    return ", ".join(
        flag for flag, given in (
            ...
            # 회계 opt-out 도 선언면에선 뜻이 없다 — 이 실행은 호출도 예약도 하지 않는다.
...
def _resolve_gate_command(args: argparse.Namespace, engine_repo: Path) -> int:
    ...
    with _round_ledger_lock():
        ...
        # 받아 다른 채널의 accepted 잔여·기계 확인은 보지 않는다.
...
# ── 서킷브레이커 측정 폭 (앵커) ────────────────────────────────────────────
# 서킷브레이커가 재야 하는 것은 "이 작업이 통합 브랜치에서 갈라진 뒤 남긴 변경"이다.
# 작업트리+직전 커밋 한 칸이라는 옛 폭은 dev 브랜치를 `--no-ff` merge 로 흡수하는 형상에서
# 0 에 수렴한다 — 완료 기록 시점 트리는 clean 이고 마지막 커밋이 전파/기록 커밋이면 경로
# 교집합이 비기 때문이다(실측).
#
...
def integration_anchor(
    ...
    try:
        result = _run(
            ["git", "-C", str(root), "merge-base", integration_ref, "HEAD"],
            capture_output=True, text=True, encoding="utf-8", errors="replace",
        )
    except OSError:
...
def _load_ticket_finish():
    """형제 완료 기록 엔진 — 묶음 장부 해소를 **그쪽 단일 정의**에서 빌려 온다(사본 0).

    `_load_board()` 와 같은 형제 로드 규약이다(경로 앵커·순환 import 없음·도구 standalone
    실행). 같은 값(장부 `base_branch`)을 이 파일이 따로 해석하면 리뷰 호출 폭과 완료 기록 폭이
    다른 통합 브랜치를 보게 된다 — 두 게이트가 같은 폭을 본다는 성질이 그 사본 하나로 깨진다."""
    path = Path(__file__).resolve().with_name("ticket_finish.py")
    return _load_module_from_path(
        path, "ticket_finish.py", verifier=_verify_engine_rev)


def _cluster_board_py(pm_home: Path | None) -> Path:
    """묶음 장부를 읽을 board 사본 — PM 홈이 주어지면 그 홈의 엔진 사본, 아니면 형제.

    장부 조회는 재앵커 없이 그 사본이 보는 트리를 본다 — 분리 형상에서 형제 사본을 쓰면 이
    도구가 사는 트리의 board 를 읽어 **다른 장부**를 본다(티켓 조회가 `pm_home` 을 따르는 것과
    같은 이유)."""
    if pm_home is not None:
        candidate = Path(pm_home) / ".project_manager" / "tools" / "board.py"
        if candidate.is_file():
            return candidate
    return Path(__file__).resolve().with_name("board.py")


def cluster_integration_tip(
    ...
    try:
        ...
        return None, INTEGRATION_TIP_SEAM_ABSENT_NOTE
    if not branch:
        return None, INTEGRATION_TIP_UNDECLARED_NOTE
    return branch, None


def _measure_stages(
    base: str, anchor: str | None,
...
_MEASURED_ANCHOR_REFUSAL = (
    "리뷰 호출 거부 — {ticket} 측정 폭의 기준점을 해소하지 못했습니다.\n"
    "  {reason}\n"
...
def _diff_cap_refusal(
    ...
    **같은 기준·같은 폭**(`measured_numstat_text` 한 곳)이라 두 게이트가 다른 단계·다른 제외
    ...
    호출되는 diff 가 얼마인가"라 공유 트리 wave 에서도 union 이 정답이고, 완료 기록이 묻는 것은
    "이 티켓 **구현**이 estimate 를 넘었나"라 티켓 귀속값이 정답이다(`ticket_finish.
    _ticket_diff_attribution`). 그래서 완료 기록 쪽의 귀속 보정(창 스냅샷·디렉터리 양보)을 이
    함수로 옮기면 안 된다 — 리뷰어가 받는 양을 실제보다 작게 재게 된다. 이 비대칭은 우연이
    아니므로 회귀가 명시 고정한다.
    ...
    실행마다 다른 폭을 재고, 과소 측정이 리뷰 호출을 통과한다.
...
def extract_diff(
    ...
    """git diff <base> -- <paths> 원문을 그대로 돌려준다.

    리뷰어가 볼 수 있는 것과 PM 이 볼 수 있는 것을 다르게 두지 않는다 — 경로를 골라내 빼는 필터는
    없다. 검토 범위는 인자(`--paths`/ticket touches)가 정한다.
...
class _TicketTouches(list[str]):
    ...
    빈-diff 게이트를 지난 뒤 같은 파일의 본문을 읽는다. plain list가 주입된 테스트
...
def _load_ticket_rounds_for(
    ...
    — 리뷰 입력이 반쪽이 된 채로 나가면 안 된다(호출부가 loud 하게 멈춘다).
    """
...
def _load_relay():
    ...
    pm_delegate 가 이 모듈을 deep-import 해 재앵커를 재사용하는 선례의 대칭 방향이다
...
def _watchdog_reviewer_run(argv, *, input=None, timeout=None, idle_timeout=None,
    ...
    `cwd`/`env` 는 리뷰어 실행 조건이다 — 시그니처에 명시해 relay 워치독까지 내려보낸다
    (`**_ignored` 가 삼키면 실행 조건이 조용히 사라진다).

    `on_spawn_attempt` 도 같은 이유로 명시한다 — 이 러너의 **앞부분**(relay 로드·프로필 해소·
    워치독 준비)은 자식이 아직 없는 구간이라, 소유권 이전 콜백을 이 함수 입구에서 태우면 스폰 0·
    호출 0 으로 끝난 준비 실패가 예산을 먹는다. 콜백은 relay 워치독의 실 스폰 경계까지 그대로
    내려간다."""
...
def _reviewer_run_kwargs(run_fn: Callable, argv: list[str], *,
    ...
    `cwd`/`env`(리뷰어 실행 조건)는 **값이 있을 때 무조건** 넘긴다 — 조건부로 넘기면 그 키를
    선언하지 않은 runner 에서 실행 조건이 조용히 사라져(silent degrade) 위임과 갈린다.
    받지 못하는 runner 는 아래 bind 검사가 loud seam 오류로 세운다. 값이 없는 실행(None)의 kwargs 는
    종전과 바이트 단위로 같다.
    """
    kwargs = {
        "input": prompt,
...
# exec 자체가 실패해 자식이 **뜬 적 없는** 확정 기동 실패 — 호출 0·과금 0 이라 예약 환불 대상이다.
...
def _started_after(exc: BaseException, seam_reached: bool) -> bool:
    ...
    실행은 이미 호출됐을 수 있고, 반대로 콜백이 돈 뒤라도 relay 가 '자식 없음'을 확정했으면 호출
...
def _run_reviewer_ex(
    ...
    *,
    ...
    """run_reviewer 본체 + 자식 프로세스 스폰 여부(started) 신호.

    반환: (성공 여부, `ReviewerOutput`(회신/로그 **분리 보관**), started). started=False = 자식 프로세스가 *확실히 시작되지
    않음*(호출 0·과금 0) — 빈 reviewer_cmd·seam 계약 오류·**스폰 경계 전의 준비 실패**·확정 기동
    실패(exec 이 실패한 `FileNotFoundError`·`PermissionError`·`NotADirectoryError`·
    `IsADirectoryError`). started=True = 스폰됨(프롬프트가 호출·과금됐을 수 있음) — 정상 종료
    (비-0 rc 포함)·타임아웃·스폰 뒤의 실행 오류. 타임아웃/불확실은 이미 호출됐을 수 있으므로
    보수적으로 started=True — 라운드 환불은 started=False 일 때만 해(반복 타임아웃으로 상한을
    무한 우회하지 못하게). 확실히 호출 전인 경우만 환불한다.
    ...
    **판정 기준**: 기본 러너가 `subprocess.run` 단일 호출에서 pm_relay 공용 워치독으로
    ...
    · 선언한 러너(기본 워치독 러너 — `on_spawn_attempt` 키워드를 이름으로 가짐)면 콜백을 **러너
      ...
      이 함수는 `started=False` 로 돌아간다 — 스폰 0·호출 0 인 실행이 예산을 먹지 않는다.
    ...
    # 없으므로 호출 0 이고, 경계 뒤면 프롬프트가 이미 나갔을 수 있다.
    spawn_reached = False
    ...
    try:
        ...
        # 러너 입구~Popen 사이의 준비 실패는 스폰 0·호출 0 이라 예약이 환불 대상으로 남는다.
        ...
        # 프로세스가 시작돼 실행 중 타임아웃 — 프롬프트가 이미 호출·과금됐을 수 있다 → started=True.
        ...
        # 호출 전 bind 실패 — 자식 프로세스는 확실히 시작되지 않았다. 라운드 환불 가능.
        ...
        if _launch_failed_definitely(exc):
            ...
            # 자식이 없었으므로 호출 0·과금 0 이고 환불 대상이다.
            return (False,
                    ReviewerOutput(_launch_failure_output(argv, exc, reviewer_cmd)),
                    False)
...
# ── 회신 품질 검출 (판정 확정 가능성) ─────────────────────────────────────
#
# "이번 판정이 이 리뷰의 것인가"를 회신 본문의 증거로 판정한다: 회신에 옛 리뷰 raw 파일명이나
# 하네스 세션 전사 경로가 인용돼 있으면 그 판정은 이번 라운드의 산출이 아닐 수 있고, 옛 판정
# 블록을 통째로 옮겨 오면 판정 라인이 여러 개가 되어 파서가 고른 한 줄이 이번 판정이라는 보장이
# 사라진다. 이것은 **출력 품질** 검사이지 접근 권한이 아니다 — 입력은 회신 텍스트 하나뿐이고
# 하네스로 갈리지 않는다.
# 검사 대상은 **회신 채널만**이다(`ReviewerOutput.answer`) — 진행 로그에는 프롬프트와 diff 원문이
# 그대로 실려 있어 그것까지 세면 정상 실행이 전부 걸린다(라이브 실측).
...
def _abort_pre_spawn_raw(
    ...
    try:
        _write_reserved_output(
            raw_path,
            f"[스폰 전 중단 — 자식 프로세스 시작 없음(호출 0·과금 0): {reason}]",
            local_conf_path=local_conf_path,
            resolved_profile=resolved_profile,
            target=target,
            )
...
def run_review(
    ...
    `contamination` = 회신 본문 품질 진단(엇갈린 판정 라인·옛 raw 파일명·전사 경로 인용).
    **하나라도 잡히면** `all_pass` 를 내려 '판정 불명확'(보수적 exit 1)로 만든다 — 이번 판정이
    이 리뷰의 것이라는 보장이 없는 회신에서 통과가 나가는 false-green 을 기계가 막는다.

    `on_spawn_attempt` = 라운드 예약 소유권 이전 seam(기본 None = 종전 동작). 이 함수의 **앞부분**
    (raw 선점·장부 시작 레코드·구조화 transport·argv/kwargs 준비)과 러너 안의 준비 구간
    (relay 로드·프로필 해소·워치독 셋업)은 아직 확실히 호출 전이라,
    ...
    # ── 여기부터 스폰 시도 seam 까지가 "확실히 호출 전" 구간이다 ──────────
    ...
    try:
        ...
        with _structured_transport(target, prompt, cwd) as (argv, stdin_text):
            ...
            if not started:
                ...
                # 실측 축이다) 호출 0·과금 0 인 실행이 예약을 미완으로 남기지 않는다.
        ...
        # 취급은 종전과 같다(판정 없음 = 미완 라운드 · 호출은 있었으므로 환불 없음).
        ...
        # 실패는 종전 보수 규칙 그대로다: 호출됐을 수 있으니 예약도 미마감 레코드도 건드리지
...
def print_summary(result: dict, gate: str | None = None) -> None:
    ...
    for marker in result.get("contamination") or ():
        print(f"  ⚠ 회신 품질 의심(이번 판정 확정 불가): {marker}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="추가 리뷰어 래퍼 — 추가 리뷰어 하네스 어댑터 CLI",
...
과금 상한: 호출마다 비용을 다시 묻지 않는다 — 무한 라운드는 라운드/wave 예산이 기계로 막는다.
""",
    )
    ...
    parser.add_argument("--pm-verified", action="store_true",
                        help="--resolve-gate 처분: PM 판정 표면(리뷰 delta)의 기계 확인 증거로 "
                             "해소 — 재호출 없음(`pm_delegate rounds resolve --pm-verified` "
...
# ── local.conf 호출 프로필 provenance/divergence ───────────────────────────
# `_find_repo_root` 보유 도구 기계 inventory: contradiction_lint.py,
# additional_reviewer.py, pm_delegate.py, ticket_finish.py. 이 중 local.conf 로 호출 대상을 고르는
# 표면은 additional_reviewer(reviewer_cmd)와 pm_delegate(delegate.*)뿐이다. ticket_finish는 local.conf의
# 완료/회귀 노브만 읽고, contradiction_lint는 local.conf 자체를 읽지 않아 이번 호출 분기 범위에서
...
def _review_raw_content(
    ...
    실행 커맨드)을 한 줄씩 더해, raw 하나만 봐도 "무엇이 이 판정을 냈는가"가 닫힌다."""
    if local_conf_path is None and resolved_profile is None and target is None:
        return content
...
class LocalConfDivergence(NamedTuple):
    """엔진 conf와 호출 대상 repo conf의 실제 호출 프로필 차이."""
...
class TargetLocalConfReadError(RuntimeError):
    """존재하는 비교 대상 local.conf를 안전하게 읽지 못해 호출을 중단해야 함."""

    def __init__(self, path: Path, cause: BaseException):
        self.path = path
        self.cause = cause
        super().__init__(
            f"대상 local.conf 읽기 실패 — 호출 전에 중단합니다: {path} "
...
def reviewer_profile_config(conf: dict[str, str]) -> dict[str, str]:
    """additional_reviewer의 실제 호출 대상 값. 미지정과 명시 default는 같은 값으로 정규화한다.
...
def local_conf_divergence(
    *,
    ...
    호출 프로필 키만 하므로 test_cmd 같은 무관 per-clone 차이는 loud 조건이 아니다.
    """
...
def format_local_conf_divergence(
    ...
    """두 호출 표면 공용 never-block loud 경고 문구."""
    ...
    return (
        ...
        "REPO가 다르고 호출 프로필/내용 값이 실제로 갈립니다.\n"
...
def _main(argv: list[str] | None = None) -> int:
    ...
    # 게이트 이름 검증이 **가장 먼저**다 — 예약 키 충돌은 호출·장부 접근 뒤에 발견하면 이미
    ...
    # 썼는데 안 먹었다"가 남아 규율이 흐려진다. 부작용 0 지점(장부 접근·호출 전)에서 거부한다.
    if args.ack_rounds:
    ...
    if args.resolve_gate:
        # 장부 기록 전용면 — 실행 경로(conf 해소·diff 추출)보다 앞에서 끝낸다.
        return _resolve_gate_command(args, engine_repo)
    if args.rounds_report:
        ignored = ", ".join(
            flag for flag, given in (
                ("--ack-wave", args.ack_wave),
                # 조회면은 호출도 예약도 없어 회계 자체가 없다 — opt-out 도 무시 대상이다.
    ...
    try:
        ...
        # 라운드 상한)는 모두 해소 뒤라 읽는 시점만 옮겨지고 값의 출처는 종전과 같다.
        ...
        if ticket_selected:
            ...
            # 인자 없는 실행 — 이 config가 **범위의 원천**이라 파생 전에 읽는다.
        ...
        if scope_from_initial_pm_home and pm_home != engine_pm_home:
            raise AnchorResolutionError(
                ...
                "config provenance와 실제 호출 범위를 일치시킬 수 없어 호출 전에 중단합니다: "
    ...
    if args.rounds_report:
        ...
        # 실행 경로(conf 해소·diff 추출)는 타지 않는다.
        return _print_rounds_report(pm_home, gate=args.gate, resolved=True)
    if conf is None:
        # 명시 앵커 실행의 config는 여기서 **처음이자 유일하게** 읽는다. 인자 없는 실행은 범위 파생
        # 지점에서 이미 읽었다.
        try:
            ...
            print(
                "오류: 해소된 local.conf 읽기 실패 — 실행 전에 중단합니다: "
    ...
    # output-dir 생성·라운드 예약·raw 예약·과금 문구 어느 것도 만들기 전에 끊는다.
    # 여기까지의 앵커 해소는 읽기 전용이고, 이 함수 자체도 부작용이 없다.
    try:
        target = resolve_reviewer_target(conf)
    except ReviewerTargetError as exc:
        print(
            f"오류: 추가 리뷰어 대상 해소 실패 — 호출 전에 중단합니다: {exc}\n"
    ...
    # 라운드 상한 거부처럼 **리뷰어를 부르지 않고 끝나는 실행**이 요청한
    ...
    try:
        diff = extract_diff(args.base, paths)
    ...
    # 플래그 없음. 기존 오류 규약(비-0 = 1)과 정합. dry-run 보다 앞서므로 잘못된 형상(worktree
    # 아닌 곳에서 실행 등)은 리뷰어를 부르지 않고 미리보기 단계에서도 드러난다.
    if not diff.strip():
        print(_empty_diff_guidance(paths, root=diff_root), file=sys.stderr)
        return 1

    # 티켓 본문은 실제 프롬프트가 필요한 dry-run 또는 호출 확정 구간에서만 읽는다.
    # diff-cap/앵커 차단은 본문 파일이나 byte 상한을 건드리지 않는다. ticket touches를
    ...
    if args.dry_run:
        ...
        # 미리보기는 **부작용 0**이다(호출·raw 예약·라운드 예약·`--output-dir`
        # 생성 모두 없음). 여기까지의 준비는 전부 읽기 전용이고(conf 해소·git diff),
    ...
    # 멈추고 사용자에게 보고한다. 자리는 **유료 호출이 확정된 구간**이라 미리보기(dry-run)는 이
    # 검사를 지나지 않는다: 미리보기는 비용이 애초에 없고, 그것을 막으면 분할 판단에 필요한 diff
    # 확인 채널까지 닫힌다.
    ...
    # raw 선점보다 **먼저**다 — 이미 상한에 닿은 호출이 raw 자리를 만들었다 지우면 "호출 없이
    # 끝나는 실행은 부작용 0" 규율(dry-run·diff-cap 거부와 같은 축)을 이 rc 에서만 깨는 것이다.
    # dry-run·조회·처분은 이미 반환했고 diff-cap 거부도 호출 없이 종료됐다. 여기까지 온 실행은
    # 리뷰어를 부르므로, `--gate`(또는 `--ticket` 자동 유도) / 명시 `--no-gate` 중 하나가 반드시
    # 있어야 한다. 둘 다 없으면 예약·raw·리뷰어 스폰 전 부작용 0 지점에서 fail-loud 한다.
    ...
    # 모든 무호출 조기 종료를 지난 뒤, 라운드 예약(부작용) 전 마지막 입력 경계에서 본문을 읽고
    # 상한을 판정한다. 여기서 실패하면 호출·라운드/wave 소비 모두 0이다.
    if not prepare_ticket_body():
        return 1
    prompt = compose_prompt()
    ...
    # 스폰 전에 중단하는 실행은 호출이 없으므로 방금 잡은 예약을 그 자리에서 원자 환불한다
    # (마감 시점 `started=False` 환불과 같은 조건·같은 기계). 환불 소유는 이 seam 하나다.
    with _PreSpawnReservation(budget) as reservation:
        return _run_review_round(
            args, conf=conf, prompt=prompt, reviewer_cmd=reviewer_cmd,
            timeout=timeout, idle_timeout=idle_timeout, output_dir=output_dir,
            conf_path=conf_path, profile=profile,
            target=target, reservation=reservation,
            pm_home=pm_home, diff_root=diff_root,
        )
...
def _run_review_round(
    args, *, conf, prompt, reviewer_cmd, timeout, idle_timeout,
    output_dir, conf_path, profile, target=None,
    reservation: "_PreSpawnReservation | None" = None,
    pm_home: Path | None = None, diff_root: Path | None = None,
) -> int:
    """예약 수명 **안**에서 도는 구간 — 리뷰 실행·장부 마감.

    `_main` 에서 떼어낸 이유는 예약 소유권 하나다: 여기서 어떤 경로로 빠져나가도(조기 return·
    예외) 호출부의 단일 `__exit__` 가 스폰 전 예약을 환불한다.

    예산 확인·예약은 이 구간 **밖**에서 이미 끝났다. 넘겨받은 예약(`reservation`)의 앞부분(환경 준비, 그리고
    ...
    # 추가 리뷰어의 실행 조건은 위임과 같다 — cwd 는 검토 대상 저장소, env 는 위임 채널이 소유한
    # 같은 seam 이 조립한다. 채널이 다르다고 다른 홈·다른 allowlist 를 만들지 않는다(그 차등은
    # 강제력 없이 비용만 받았다: 리뷰어는 절대경로를 그대로 읽을 수 있고, 임시 홈은 세션 재사용을
    # 막아 라운드마다 전체 payload 를 다시 과금했다).
    reviewer_environment = (
        _load_pm_delegate().build_env(target.harness) if target is not None else None
    )
    print(
        f"[additional-reviewer] 리뷰어 실행 조건: cwd={diff_root} "
        "· env=위임 채널과 같은 seam(pm_delegate.build_env)",
        file=sys.stderr,
    )

    print(f"추가 리뷰어 실행 중 (유료 호출): {reviewer_cmd}", file=sys.stderr)
    ...
    # 여기서 미리 넘기면 요약 출력이 닫힌 stdout 파이프로 죽는 순간(BrokenPipeError) 호출 0·과금 0
    ...
    if budget.reserved:
        ...
        try:
            with _round_ledger_lock():
                ledger = _load_round_ledger()
                if not started:
                    # 호출이 확실히 없던 라운드 — 두 예산(게이트 count·wave spent)을 같은 조건으로
                    # 되돌린다(스폰 전 실패 환불과 **같은 기계**). 산출도 남기지 않는다
                    # (리뷰어가 아무 말도 하지 않았다).
                    ...
                    # 산출 기록은 예약 레코드 유무와 무관하다 — 호출된 라운드는 무조건 남긴다
            ...
            if not started:
                ...
                if refunded_round:
                    print(
                        ...
                        "추가 리뷰어 교차검증이 **실행되지 않았습니다**(호출 0·상한/wave 예산 "
            ...
            # 마감은 이미 *끝난* 호출의 기록이다 — 여기서 rc 를 바꾸면 리뷰 판정이 락 사정으로
            ...
            # 못한다: 그대로 두면 호출 0·과금 0 인 실행이 라운드·wave 예산을 먹은 채 미완으로
            ...
            print(
                ...
                + ("호출이 없던 라운드라 예약은 되돌렸습니다(상한·wave 예산 미소진)."
                   if refunded else
    ...
    if target_problem is not None:
        ...
        print(
            ...
            "리뷰어 회신을 받지 못했습니다(호출 실패·타임아웃). 산출 없음이라 라운드 파일을 "
