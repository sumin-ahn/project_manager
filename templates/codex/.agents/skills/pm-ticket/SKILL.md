...
python3 .project_manager/tools/board.py new "<한 줄 제목>" \
  ...
  [--estimate small|medium|large] [--design required|n/a] [--prefix <카테고리>]
```
...
- `--design`: 설계 단계 상태. 생략 시 `--estimate large`면 `required`, 그 외는 `n/a`로 박힌다. 설계 검증이 필요하다고 판단하면 small/medium에도 `--design required`로 명시 지정한다.
- 출력의 `created <T-NNNN>`로 ID를 확인한다.

## 2. 본문 6절 자족성 채움

`board.py show <T-NNNN>`로 발행 파일 경로를 확인하고 6절을 실값으로 채운다. dev 서브에이전트가 본문만으로 추측 없이 구현할 수 있도록 실값 경로와 숨은 전제를 명시하고 placeholder를 0으로 만든다.
...
- 설계 절이 미충전이거나 필드가 `required`로 남아 있으면 `claim`이 rc=1로 거부하고, 전역 `lint`는 `design-pending` 경고 1줄(never-block)로 보여준다.
...
- `promote`는 승격 전 본문을 재검사한다. placeholder(5절 뼈대 문장·`xxxxx`·`T-XXXX` 등)나 누락 표준 절이 있으면 **거부**(rc=1)하고 이슈를 나열한다. `design: required`면 설계 절 미충전도 같은 자리에서 거부된다. draft는 `.drafts/`에 남으므로 더 채운 뒤 재시도한다.
- 통과하면 draft → `open/` 이동 + board-git 커밋(공유 board에 등장) 후 claim 가능해진다(단, `design: required`가 남아 있으면 claim 게이트가 따로 거부한다).
...
- `.project_manager/wiki/tickets/_template.md`: 6절 티켓 뼈대(목표/인터페이스/결정/설계/DoD/참고) + `design:` 필드.
