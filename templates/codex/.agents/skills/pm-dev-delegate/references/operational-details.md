...
- 생성 실패는 **호출 전** 차단이다. 실패 사유는 생성기가 그대로 낸다(스냅샷 없는 리뷰로 강등하지
  않는다).
- 추가 리뷰어 채널(`additional_reviewer.py`)은 staged diff 기반이라 이 스냅샷 축과 별개다 — 검토 파일을
...
- `pm_delegate --role developer --ticket T-NNNN`(dry-run 포함)은 호출 전에 **같은 세션이 claim
