...
# ── run()/run_trigger() e2e guard — REPO monkeypatch 로 모호 형상 ──────────────
...
def test_run_trigger_aborts_loud_on_ambiguous_multipm(handoff, tmp_path, monkeypatch, capsys):
    """bare run_trigger() + 멀티-PM 모호 → rc 1·명시 에러·log 무접촉."""
    monkeypatch.setattr(handoff, "REPO", tmp_path)
    _make_multipm_ambiguous_repo(tmp_path)
    inst, log_file = _bare_handoff(handoff, tmp_path)
    before_log = log_file.read_text(encoding="utf-8")

    rc = inst.run_trigger(reason="ctx-threshold", ctx_pct=92, dry_run=False)
