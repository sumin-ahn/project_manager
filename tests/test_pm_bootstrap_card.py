...
# ── 12. 보드 요약 open 라벨 정정 + 타 세션 claim 현황 (T-0331 · DoD) ──────────────
#
# 부트스트랩 Board 섹션의 두 정정을 헬퍼 단위로 못박는다(순수 함수·I/O 0):
#   1. open 카운트 라벨 = `(공유 backlog·슬롯무관)` (done/claimed/blocked 는 slot-scope 유지).
...
def test_board_counts_open_label_is_shared_backlog(bootstrap):
    ...
    assert "blocked: 0 (slot 1)" in line
    # open 이 옛 slot-scope 라벨을 달지 않는다(정정 회귀 가드).
...
def test_other_session_claims_present_case(bootstrap):
    """interface 2 (유): 타 세션 claimed 를 세션별로 묶고 내 세션은 뺀다 — 현황 줄 생성."""
    output = "".join(
        f"  [{status:7s}] {tid}  {title[:60]:60s}  {claimed:18s}  {tags}\n"
        for status, tid, title, claimed, tags in [
            ("claimed", "T-A-010", "wip mine", "alice/project_manager_1", "-"),
            ("claimed", "T-A-020", "wip slot2", "alice/project_manager_2", "-"),
            ("claimed", "T-A-021", "wip slot2b", "bob/project_manager_2", "tag1"),
            ("open", "T-A-030", "backlog", "", "-"),
        ]
    )
    claims = bootstrap.parse_other_session_claims(output, "project_manager_1")
    # 내 세션(project_manager_1)은 빠지고, 타 세션(project_manager_2)만 그룹(user 무관).
    assert claims == {"project_manager_2": ["T-A-020", "T-A-021"]}
    line = bootstrap._format_other_session_claims_line(claims)
    assert line == "- 타 세션 진행(claimed): 2건 — project_manager_2: T-A-020, T-A-021"


def test_other_session_claims_absent_case(bootstrap):
    """interface 2 (무): 타 세션 claimed 0건이면 빈 dict → 현황 줄 None(생략)."""
    output = (
        "  [claimed] T-A-010  wip mine     alice/project_manager_1   -\n"
        "  [open   ] T-A-030  backlog      -                         -\n"
    )
    claims = bootstrap.parse_other_session_claims(output, "project_manager_1")
    assert claims == {}
    assert bootstrap._format_other_session_claims_line(claims) is None


def test_other_session_claims_solo_none_session_yields_empty(bootstrap):
    """my_session 미해소(진짜 솔로·None)면 내 것을 세션으로 못 가려 전부 제외(빈 dict·줄 생략)."""
    output = "  [claimed] T-A-010  wip x   alice/project_manager_1   -\n"
    assert bootstrap.parse_other_session_claims(output, None) == {}


def test_other_session_claims_multi_session_grouping(bootstrap):
    """여러 타 세션이면 세션별 그룹을 ` · ` 로 병기하고 총건수를 센다."""
    output = "".join(
        f"  [{'claimed':7s}] {tid}  {title[:60]:60s}  {claimed:18s}  -\n"
        for tid, title, claimed in [
            ("T-A-020", "a", "alice/project_manager_2"),
            ("T-A-030", "b", "carol/project_manager_3"),
        ]
    )
    claims = bootstrap.parse_other_session_claims(output, "project_manager_1")
    line = bootstrap._format_other_session_claims_line(claims)
    assert line == (
        "- 타 세션 진행(claimed): 2건 — "
        "project_manager_2: T-A-020 · project_manager_3: T-A-030"
    )


def test_parse_other_session_claims_title_with_double_spaces(bootstrap):
    """codex must-fix 1: 제목에 연속 공백(2칸+)이 있어도 claimed_by 를 오추출하지 않는다 —
    `\\s{2,}` split 이 아니라 *정체성 토큰 앵커*(행끝에 닿는 `<user>/<session…_N>`)라서. 제목
    내부의 `_\\d+`-끝 유인 단어(`slot_2`)도 배제(뒤에 claimed·tags 가 더 있어 EOL 에 못 닿음)."""
    title = "fix  slot_2  race  bug"  # 이중 공백 3곳 + slot_2(오추출 유인)
    claimed = "alice/project_manager_2"
    # board.py cmd_list 60-폭 포맷 재현(제목만 이중 공백 주입).
    line = f"  [{'claimed':7s}] T-A-070  {title[:60]:60s}  {claimed:18s}  ux\n"
    claims = bootstrap.parse_other_session_claims(line, "project_manager_1")
    assert claims == {"project_manager_2": ["T-A-070"]}, "제목 이중공백에 claimed_by 오추출"


def test_parse_other_session_claims_tag_with_slash(bootstrap):
    """codex R2 must-fix: tags 는 board.py 가 검증 없이 join 하므로 `/` 포함(`area/foo`)이 합법 —
    tags charclass 가 `/` 를 못 받으면 행 전체 매칭 실패로 그 티켓이 *조용히 누락*된다.
    tags suffix 는 공백 없는 임의 토큰(`\\S+`)으로 수용."""
    line = f"  [{'claimed':7s}] T-A-080  {'harden parser':60s}  {'bob/project_manager_3':18s}  area/foo,v1.2.1\n"
    claims = bootstrap.parse_other_session_claims(line, "project_manager_1")
    assert claims == {"project_manager_3": ["T-A-080"]}, "slash tag 행 누락"


def test_parse_other_session_claims_legacy_session_and_underscore_tag(bootstrap):
    """codex R3 must-fix: legacy 세션형(`pm-2`·비-`_N`) claimed_by + `_N`-꼬리 tag(`area_2`) 조합에서
    tag 를 세션으로 오인하지 않는다 — 고정폭 위치 파싱이라 tags 컬럼은 아예 읽지 않고, legacy
    claimed_by 도 컬럼 위치에서 그대로 세션으로 수용된다."""
    line = f"  [{'claimed':7s}] T-A-090  {'harden parser again':60s}  {'bob/pm-2':18s}  area_2\n"
    claims = bootstrap.parse_other_session_claims(line, "project_manager_1")
    assert claims == {"pm-2": ["T-A-090"]}, "legacy 세션형 누락 또는 _N-tag 세션 오인"


# ── board.py cmd_list 실행 통합 가드 (should-fix — 포맷 복사본 coupling 제거) ────────
#
# 포맷 문자열의 *복사본*에 묶인 가드는 board.py 포맷이 바뀌면 silent-0 이라, board.py cmd_list 를
# **실제 실행**한 stdout 을 파서에 먹여 coupling 을 못박는다(must-fix 2 로 전용 무렌즈 조회가 생긴
# 실배선 경로와 동일 데이터 형태). 헤르메틱 board 는 test_board_scoping_isolation 패턴 동형.


def _make_board_project(root: Path) -> None:
...
def _hermetic_board(tmp_path, monkeypatch):
    """모든 IO 전역을 tmp 프로젝트로 재지정한 hermetic board 모듈(실 장부·git 미접촉)."""
    proj = tmp_path / "proj"
    _make_board_project(proj)
    mod = _load("board")
    pm = proj / ".project_manager"
    wiki = pm / "wiki"
    local = pm / ".local"
    for name, val in {
        "REPO": proj, "TICKETS_DIR": wiki / "tickets",
        "TEMPLATE_FILE": wiki / "tickets" / "_template.md",
        "BOARD_FILE": wiki / "board.md", "LOG_FILE": wiki / "log" / "current.md",
        "STATUS_FILE": wiki / "status.md", "LOCAL_CONF": pm / "local.conf",
        "AREAS_FILE": pm / "areas.md", "LOCAL_DIR": local,
        "LEASES_FILE": local / "worktree-leases.json", "BOARD_LOCK": local / "board.lock",
...
def _seed_claim(board_mod, capsys, *, session, user, title) -> str:
    """실 cmd_new + cmd_claim 으로 claimed 티켓을 만들고(claimed_by=<user>/<session> 스탬프) id 반환."""
    repo, _, num = session.rpartition("_")
    rc = board_mod.cmd_new(argparse.Namespace(
        prefix=None, session=session, user=user, title=title,
        touches=None, depends=None, tag=None, estimate="small"))
    out = capsys.readouterr().out
    assert rc == 0, out
    tid = re.search(r"created (T-\S+)", out).group(1)
    rc = board_mod.cmd_claim(argparse.Namespace(id=tid, repo=repo, slot=int(num), user=user))
    assert rc == 0, capsys.readouterr().out
    capsys.readouterr()  # 버퍼 클리어(다음 cmd_list stdout 만 캡처).
    return tid


def test_parse_other_session_claims_against_real_board_cmd_list(
    bootstrap, tmp_path, monkeypatch, capsys
):
    """should-fix: board.py cmd_list 를 **실제 실행**한 stdout 을 파서에 먹인다(포맷 복사본 coupling
    제거·board.py 포맷 변경 시 red). 두 세션(alpha_1·alpha_2)이 claim → 무렌즈 `list --status
    claimed` stdout → 파서가 내 세션(alpha_1)은 빼고 타 세션(alpha_2·동일 사용자 타 슬롯)만 잡는지."""
    ...
    claims = bootstrap.parse_other_session_claims(out, "alpha_1")
    # 내 세션(alpha_1) 제외 · 타 세션(alpha_2)만 — 동일 사용자(alice) 타 슬롯도 병기.
    assert claims == {"alpha_2": [other]}
