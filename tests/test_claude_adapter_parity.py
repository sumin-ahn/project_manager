...
def test_claude_native_ticket_rounds_use_agent_contract_for_hand_spawned_roles():
    """Agent의 실제 필드와 손-스폰 역할의 라운드 파일 지시를 prepare→harvest 사이에 고정한다.

    라운드 파일 모델(ADR-0090)에서 각 위임이 쓸 수 있는 파일은 `NN-<역할>.md` 하나뿐이라,
    프롬프트 블록마다 그 역할의 파일 이름과 prepare 출력 경로가 함께 실려야 한다.
    손-스폰 역할은 둘(developer·architect)이다 — 리뷰는 엔진이 스냅샷·프롬프트·라운드 자리를
    만들어 직접 스폰하므로 카드에 손 위임 블록이 없다(있으면 두 경로가 갈린다).
    """
