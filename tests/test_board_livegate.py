...
def test_record_rc_nonzero_records_fail(live_board, monkeypatch, capsys):
    """rc!=0 → status='fail' + rc1 — 수집 N==pin(16) 이어도 red 는 fail.

    rc 게이트가 수집 N 과 독립임을 확인: 16개 실행(1 red)이라 N=16 이지만 rc1 → fail.
    """
    fake = _FakeRun(1, "1 failed, 15 passed, 810 deselected in 40.0s")
    monkeypatch.setattr(live_board.subprocess, "run", fake)
    rc = live_board.cmd_livegate(_rec_args())
    assert rc == 1
    data = _read_flag(live_board)
    assert data["status"] == "fail"
    assert data["n"] == 16         # 실행은 16 이지만
