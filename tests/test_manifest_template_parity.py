...
#     pm-review file override(추가 리뷰어 실 호출의 `exec_command` 건별 egress 승격 절차 — codex 전역
...
REOPEN_LIFECYCLE_ENGINE_PATHS = (
    ".project_manager/tools/worktree_pool.py",
    ".project_manager/tools/pm_config.py",
...
def test_reopen_lifecycle_engines_are_manifested_and_shipped_byte_identical():
    """T-0873 reopen 핵심 네 엔진의 출하 membership과 세 target bytes를 직접 고정한다."""
    entries = {
        str(entry): entry
        for entry in _load_pm_update().read_manifest(ROOT_MANIFEST)
    }
    assert len(REOPEN_LIFECYCLE_ENGINE_PATHS) == 4

    for rel in REOPEN_LIFECYCLE_ENGINE_PATHS:
        entry = entries.get(rel)
        assert entry is not None, f"reopen lifecycle 엔진이 root manifest에서 누락됨: {rel}"
        assert not entry.render and not entry.target_owned, (
            f"reopen lifecycle 엔진은 byte-copy 대상이어야 함: {rel} "
            f"(render={entry.render}, target_owned={entry.target_owned})"
        )
        canonical = REPO / rel
        assert canonical.is_file(), f"reopen lifecycle canonical 엔진 부재: {rel}"
        for target, template_root in TEMPLATE_ROOTS.items():
            shipped = template_root / rel
            assert shipped.is_file(), f"{target} reopen lifecycle 출하 파일 부재: {rel}"
            assert shipped.read_bytes() == canonical.read_bytes(), (
                f"{target} reopen lifecycle 출하 파일 byte drift: {rel}"
            )
