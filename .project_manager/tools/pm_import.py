...
def record_upstream(dest_root: Path, source_root: Path) -> bool:
    """import source(--from)의 절대경로를 dest local.conf 에 `upstream=` 로 기록한다(T-0053).
    ...
    쓰지 않으므로 preserve 가 옛 값을 복원한다) 마지막에 *현재 source* 로 제자리 확정 갱신된다(stale
    ...
    if not local_conf.is_file():
        ...
        return False
    text = local_conf.read_text(encoding="utf-8")
    new_text = _set_conf_keys(text, {"upstream": str(source_root)})
    if new_text != text:
        local_conf.write_text(new_text, encoding="utf-8")
        return True
    return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        ...
        epilog=(
            "upstream 기록: --from 경로는 이 프로젝트의 .project_manager/local.conf 에 "
            "`upstream=` 로 기록돼, 이후 pm_update 가 --from 생략 시 자동 사용한다 "
            "(재-import 시 현재 --from 으로 갱신)."
        ),
    )
    ...
    ap.add_argument("--from", dest="source", default=str(REPO),
                    help="프레임워크 checkout 경로 (default: 이 repo 루트)")
    ...
    # T-0053: import source(--from)를 local.conf 에 upstream= 으로 기록한다. board init·conf
    #   sync·preserve 단계 *이후* 에 둬야 한다 — 그래야 재-import 에서도 preserve 가 stale 값을
    #   붙들지 않고 *현재 source* 로 확정된다(_set_conf_keys 제자리 갱신). 이후 pm_update 가
