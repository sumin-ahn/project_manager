...
def _resolve_log_file(repo: Path = REPO) -> Path:
    """공유 `log/current.md` 경로 — worktree 슬롯 cwd 에서도 PM 홈 공유 로그를 가리킨다.

    자기분리 토폴로지에서 board/wiki/log 는 PM 홈이 소유하고 코드/tests 는 슬롯
    ...
    *실재할 때만* 그걸 가리킨다. 슬롯이 아니거나(PM 홈 직접 실행) 상위 공유 로그 부재면
...
# worktree 리스 장부 — worktree_pool.LEASES_FILE 와 *같은 위치*. _auto_slot 이
...
#   - `render`  = 카드에 찍히는 정확한 인자 문자열(정체성/task 명 suffix 는 caller 가 보간
...
# 정의서로 단일화하고, 파서 정합은 introspection(`_registered_leaves` 방식 재사용)으로 못박는다.
...
# 슬롯 카드 CLI 커맨드 (현행 표면·byte 동일) — 정체성 suffix 는 render 에 미포함(caller 보간).
_C_BOARD_LIST_MINE = _CardCmd("board.py", "list --mine", ("list",), ("--mine",))
_C_BOARD_LIST = _CardCmd("board.py", "list", ("list",), ())
# `list --all`— 무인자 기본이 세션 스코프(내 세션 스트림만·타 세션분
...
# task 모드 CLI 커맨드 — task 명은 정체성 축이라 alloc/release/wave 는
...
_CARD_TASK_CLI = (
    # `_C_BOARD_LIST` = task-스코프 뷰 `list --task <이름>` — `--task`
...
# ── codex 하네스 감지 + 카드 codex 절 ──────────
# codex 전용 정적 진입 doc 이 없는 C-v2 구조에서, 부트스트랩 카드가 codex 실행모델·
...
# board/ 안(board/areas.md), 아니면 legacy `.project_manager/areas.md`. 미분리면 현 위치
# 100% 폴백(회귀 0). 상수 AREAS_FILE 는 hermetic 테스트 seam·legacy 기본값으로 유지.

def _areas_file() -> Path:
    """areas 레지스트리 경로 (board.py `areas_file` 동형·board_root 추종).

    `.project_manager/board/tickets` 가 실 디렉토리면 board 가 submodule 로 분리된 형상
    areas 도 board/ 안(`board/areas.md`). 아니면 legacy wiki-밖 위치
...
# multi-PM 인자(--repo)를 받았을 때만 alloc 경로에 진입한다. 무인자 경로는 이
# 모듈을 전혀 쓰지 않으므로 import 실패가 무해(fail-soft) — 단 --repo 를 줬는데
# worktree_pool 이 없으면 **명시 에러**(침묵 무력화 금지).
def _load_worktree_pool():
    ...
    스크립트-위치 앵커 관례. alloc 미진입 경로에선 호출 안 되거나 None 이어도
...
def _load_tool(name: str):
    ...
    파싱(`pm_log.split_entries`)을 **DRY** 로 재사용하기 위한 역방향 동적 로드
...
def _load_identity_args():
    ...
    없음"과 동일하게 흡수해 bare-bootstrap 경로를 깨지 않는다(B-1).
    """
...
def _registered_repos(areas_file: Path | None = None) -> list[str]:
    ...
    칼럼을 stdlib 로 가볍게 읽는다. 파일 부재/스키마 불일치 → 빈 목록(fail-soft·무해).
    `areas_file` 미지정이면 `_areas_file()`(board_root 추종)로 *호출 시점* 해소.
    """
...
def _repo_slot_numbers(repo: str, leases_file: Path) -> list[int] | None:
    ...
    **공용 `identity_args.repo_slot_numbers` 위임** — 리스
...
def _auto_slot(
    ...
    무인자 bootstrap(`--repo`/`--slot` 둘 다 없음)에서, 등록 repo 가 정확히 1개이고
    그 repo 의 활성 worktree 슬롯(leased lease 엔트리)이 정확히 1개면 모호함이 없으므로 그 슬롯에
    자동으로 bind 한다. 그 외는 None
    (미해소) — repo 0개/≥2개 또는 활성 슬롯 0개/≥2개면 사용자가 `--repo --slot` 명시.
    ...
    판정:
      ...
      2. **공용 `identity_args.resolve_actor_slot` 위임** —
    ...
    **idle 필터 영향(codex must-fix)**: 공용 프리미티브가 leased 만 세므로, `{1:idle, 2:leased}`
    ...
    display)이 *활성* 슬롯을 찾는 것이라 오히려 정합·개선(의도된 변화). `{1:leased}`→1 은 불변.

    파일 부재/스키마 불일치/JSON 깨짐/`identity_args` 로드 실패 → None(fail-soft — 자동바인딩은
    *추가 편의*이지 강제 아님·실패는 미해소로 폴백). `areas_file` 미지정(None)이면
...
# 이어야 하는지 명확해야 한다. 슬롯을 하나만 쓰는 홈도 그 홈 자신이 등록된 슬롯 행이므로,
# 해소 실패는 "등록 안 된 홈" 아니면 "under-specified" 둘 중 하나다 — 둘 다 명시 에러다.
# 조용한 폴백은 1→2 슬롯 경계에서 침묵 동작변경(없는 legacy 경로·연속성 단절)을 만들었다.
class SlotResolutionError(Exception):
    """슬롯을 자동해소할 수 없을 때 — session-entry fail-loud.

    사유는 둘이다: 등록 repo/슬롯이 없어 **아직 등록되지 않은 홈**이거나, 여럿이라
    under-specified 이거나. 메시지는 그 사유 + `--repo`/`--slot` 안내를 담는다(호출부가
    그대로 사용자에게 surface).
    """
...
def _resolve_session_slot(
    ...
    반환:
      ...
      - **raise `SlotResolutionError`** — 미등록(등록 repo 0개, 또는 등록 repo 1개인데 장부를
        읽을 수 없거나 그 repo 슬롯이 0개) 또는 under-specified(등록 repo ≥2, 또는 repo 1개인데
        슬롯 ≥2 이고 slot 1 부재한 비단독). 둘 다 조용한 폴백 대상이 아니다.
    ...
    미import(touches 격리) 유지. `areas_file` 미지정이면 `_registered_repos` 가
    `_areas_file()`(board_root 추종)로 해소.
    """
    repos = _registered_repos(areas_file)
    if len(repos) == 0:
        raise SlotResolutionError(
            "등록 repo 0개 — 이 홈은 아직 셋업 전이다. `pm-config init` 으로 셋업하거나 "
            "`--repo <name> --slot <N>` 으로 명시하라."
        )
    if len(repos) > 1:
        raise SlotResolutionError(
    ...
    if not slot_nums:
        # 장부 부재/깨짐(None) 또는 그 repo 슬롯 0개([]) — 이 홈은 아직 슬롯 행이 없다.
        # 조용히 폴백하면 어느 슬롯의 연속성인지 모른 채 로그·pm_state 를 쓰게 된다.
        raise SlotResolutionError(
            f"repo '{repo}' 의 활성 슬롯 0개 — 이 홈이 아직 슬롯 행으로 등록되지 않았거나 "
            f"장부를 읽을 수 없다. `pm-update` 를 1회 실행해 등록을 마치거나 "
            f"`--repo <name> --slot <N>` 으로 명시하라."
        )
    if 1 in slot_nums:
...
def _session_slot_identity(
    ...
    반환은 `identity_args.SlotIdentity`(키 + 장부 경로 값) 또는 None(미해소).
...
# pm_state 는 *슬롯별*이다 — pm_handoff 가 활성 슬롯의 pm_state 를
# read/write 하므로(`.local/slots/<slot>/pm_state.md`), 부트스트랩의
...
_PM_STATE_UNRESOLVED_DISPLAY = "(슬롯 미해소 — pm_state 경로 미확정)"
...
def _pm_state_display_path(
    ...
    """첫-turn 안내에 쓸 pm_state 경로 문자열 (per-slot).

    슬롯 정체성 키가 해소되면(`<repo>_<N>`·명시 또는 `_auto_slot` 단일 self-host) per-slot 경로
    `.project_manager/.local/slots/<키>/pm_state.md`, 미해소면 경로 대신 미확정 표기다 — 슬롯을
    하나만 쓰는 홈도 그 홈 자신이 슬롯 행이라 키가 해소되므로, 미해소는 "이 홈은 저 legacy
    파일을 쓴다"가 아니라 "아직 모른다"이고 그렇게 보여야 한다. 키는 pm_handoff 가 read/write
    하는 상태 경로와 **같은 해소**(`identity_args.resolve_slot_identity` — 장부 값 1순위)에서
    온다. `_auto_slot` 은 같은 모듈 함수라 직접 호출(동적로드 불요·`_worktree_cwd` 동형).
    예외/None 은 흡수해 미확정 표기로 폴백. `leases_file` 미지정이면 *호출 시점* REPO 기준
    재구성(monkeypatch 추종·hermetic).
    """
    if leases_file is None:
        leases_file = REPO / ".project_manager" / ".local" / "worktree-leases.json"
    key = slot_key
    ...
    if not key:
        return _PM_STATE_UNRESOLVED_DISPLAY
...
# 네트워크 git(fetch/pull) 1회당 timeout 상한 (초)
# 부트스트랩은 네트워크 I/O 를 도입했다(fetch·pull). 원격이
...
# 등록측 `pm_config._REPO_NAME_RE` 와 정합)와 *같은 문법*이어야 한다. board list --mine(
...
def parse_open_tickets(board_output: str) -> list[str]:
    ...
    `T-PAY-001`)·legacy(`T-0164`) ID 를 둘 다 파싱한다 (board.py grammar 정합).
    """
    tickets: list[str] = []
...
# 세션 기본 뷰가 타 세션분을 board.py 층에서 완전 비노출하도록 바뀌어, 부트스트랩
...
# 기본 dump 는 세션 렌즈 조회(`list --repo X --slot N`/`--mine`)의 open_tickets 를 그대로
...
def parse_lint_result(lint_output: str) -> str:
    ...
    # 우연히 이 문자열을 포함할 때 실 이슈가 있어도 clean 으로 오판한다
...
def freshness_decision(
    *, fetched: bool, detached: bool, dirty: bool | None, ahead: int | None, behind: int | None
    ...
    """fetch 후 scope 상태 문자열 + ff-pull 수행 여부를 판정한다 (순수 함수·자동 pull 안전 게이트).

    - detached HEAD → ("detached", False) — 재부착은 PM 판단·자동 pull 안 함.
...
def _behind_warning(scope: dict) -> str:
    """behind>0 인데 자동 pull 불가한 scope 의 경고 문자열 + 차단 사유.
...
def _format_freshness(scope: dict) -> str:
    """freshness scope 한 줄 표기 — fetch/behind/ahead/상태 + 동기·경고 노트.
    ...
    stale board 를 "최신" 으로 오신뢰하는 사고 차단. behind>0 은
...
def reattach_warning(current_branch: str | None, log_body: str | None) -> str | None:
    """현 worktree 브랜치가 직전 handoff 의 worktree 브랜치와 다르면 경고 문자열.
    ...
    return (
        ...
        f"직전 handoff `{expected}` (재부착은 PM 판단·자동 checkout 안 함)"
    )
...
def extract_last_log_entry_body(log_text: str) -> str | None:
    """log/current.md 의 마지막 `## [..]` entry **본문 전체**를 반환한다 (인계 dump).
...
def infer_session_num(pm_state_text: str) -> int | str | None:
    """per-slot pm_state 텍스트에서 다음 PM 세션 차수를 추론한다 (차수 announce).

    **단일 진실 = `pm_handoff.infer_next_session_num`**(세션 식별 절 최고차+1)을 동적 로드해
    재사용한다(복붙 금지·DRY). pm_handoff 부재/로드 실패면 None(소프트
...
_TASK_TAG_PREFIX = "task:"   # pm_handoff `_TASK_TAG_PREFIX` 미러 (모듈 격리라 각 모듈 inline).
...
def _session_owns_untagged(bound_session: str | None, leases_file: Path | None = None) -> bool:
    """무태그 handoff entry 가 이 세션 소유인지 판정한다 — slot-1 만 True.

    무태그 entry = 태그 도입 이전 로그 → slot-1 귀속(연속성 보존·제로 마이그레이션).
    **slot-2+ 는 무태그를 무시**하고 자기 태그 entry 만 센다(핵심 회귀 가드). 슬롯을 하나만
    쓰는 홈도 그 홈 자신이 1번 슬롯 행이라 같은 규칙 하나로 판정된다 — 별도 특례가 없다.
    **장부가 그 세션에 준 슬롯 번호가 1**이면 True, 그 외(slot-2+·번호 미해소·정체성 미해소)면
    False.
    ...
    **task 세션(`task:<name>`)은 항상 False** — task 는 자기 태그(`(task:<name>)`) entry 만
    소유하고 태그-도입 이전 무태그 legacy(slot-1 계보)는 자기 것이 아니다. sentinel 판별을
...
def _handoff_entry_owned(match: re.Match, bound_session: str | None,
    ...
    """handoff 헤더 match 가 bound_session 슬롯 소유인지 판정한다 (슬롯 필터 공용).

    태그 있는 entry → 태그 == bound_session 일 때만 소유(타 슬롯 entry 유입 차단). 태그 없는
    entry → slot-1(`owns_untagged`)만 소유. 차수 유도(`parse_last_handoff_session_num`)와
...
def parse_last_handoff_session_num(
    ...
    """log/current.md 에서 **자기 슬롯** handoff entry 제목의 최고 `PM N차` N 을 반환한다.
    ...
    무태그/타 슬롯 무시). **bound 가 None(정체성 미해소)이면 전역 tag-agnostic
    파싱**으로 폴백한다: fresh clone(lease 부재)에 tracked 로그가 태그를 가져도
    log-derived 차수를 잃지 않게(user 연속성 surface 와 동일 원리 — 양성 슬롯일 때만
...
def extract_slot_handoff_entry(
    ...
    """**자기 슬롯**의 마지막 handoff entry(date/type/title + 본문 전체)를 반환한다.

    부트스트랩 인계 dump 를 전역 마지막 entry 가 아니라 *자기 슬롯의 마지막 handoff* 로 좁힌다
    — 타 슬롯 entry 본문이 컨텍스트에 유입되면 자기 복원이 흐려진다. 슬롯 필터는
    `parse_last_handoff_session_num` 과 같은 규칙(`_handoff_entry_owned`·무태그=slot-1)이고,
    **bound 가 미해소(None)면 같은 함수와 동형으로 태그 무관 전역 파싱**이다(걸러낼 자기 슬롯이
    없는데 무태그만 고르면 태그된 최신 entry 를 잃는다).
...
def last_handoff_header_line(
    ...
    """**자기 슬롯** 마지막 handoff entry 의 헤더 줄 전체를 반환한다 (user 연속성 pickaxe needle).
    ...
    얻기 위함. `bound_session`(`<repo>_<N>`·미해소면 None)으로 **자기 슬롯 태그 entry 만 필터**한다
    ("타 슬롯 최소 유입") — slot-2 부트스트랩이 전역 마지막(=slot-1) handoff 로
    "직전 작성자" 를 오판정하지 않게. 무태그=slot-1 귀속. 부재/파싱 실패 → None.
    """
    if not log_text:
        return None
    owns_untagged = _session_owns_untagged(bound_session)
    ...
    for m in _LOG_HANDOFF_HEADER_RE.finditer(log_text):
        # bound 미해소는 자기 슬롯을 모른다 — 전역 마지막 handoff(원 동작 보존·타입/태그 무관).
...
def reconcile_session_num(
    ...
    """pm_state-derived 차수와 log-derived *다음* 차수(N+1)를 교차검증한다.
...
def extract_remaining_work_section(pm_state_text: str) -> str | None:
    ...
    인계 컨텍스트 dump 의 일부. 앵커(`## 남은 작업 전체 그림`)부터 다음 `## ` 헤더
...
# fresh 슬롯(첫 바인딩·pm_state·자기 슬롯 handoff 둘 다 부재) 명시 배너. fresh 는
...
def _format_board_counts_line(counts: dict[str, int], scope_label: str = "mine") -> str:
    ...
    done" 처럼 오독하지 않게 한다 — 무바인딩은 `"mine"`(user·전 슬롯), 명시 슬롯 바인딩
...
def _format_board_git_freshness(board_git: dict) -> str:
    """board submodule freshness 한 줄을 만든다 — HEAD·dirty·ahead/behind.
...
def _format_session_label(handoff_ctx: dict | None) -> str:
    """차수 announce 머리표를 만든다 — `PM <N>차` / placeholder (crash 금지).
...
def _format_stale_warning(handoff_ctx: dict | None) -> str | None:
    """pm_state stale 교차검증 경고 1줄 — log-derived 차수가 pm_state 보다 앞설 때.
...
# ── 슬롯 상태 surface (upstream·submodule 역할) ─────
# 부트스트랩이 현재 슬롯의 상태(branch·upstream·submodule pin/dev-ahead/drift)를 1회
# surface 한다(self-sufficient dump). 판정은 worktree_pool.slot_status(백본
# 역할 판별 재사용)가 하고, 여기선 그 결과(JSON-safe dict)를 markdown 줄로 렌더한다.
# **drift(경고) vs dev-ahead(정보) 구별**이 핵심 — dev 작업 중인
...
def slot_status_to_dict(status: object | None) -> dict | None:
    """worktree_pool.SlotStatus(백본 반환)를 JSON-safe dict 로 변환한다 (markdown/JSON 공용).
...
def _format_submodule_token(sub: dict) -> str:
    """submodule 하나를 `` `path` <라벨> `` 토큰으로 렌더한다 (경고면 ⚠·dirty 면 ·dirty)."""
...
def _format_slot_status_lines(status: dict | None) -> list[str]:
    """슬롯 상태 dict → markdown 줄 (branch·upstream + submodule 역할·submodule 없으면 줄 생략).

    `status` = `slot_status_to_dict` 반환(또는 None). None 이면 빈 리스트(절 생략·백본 미제공).
    - branch·upstream 한 줄 — upstream 미해소면 ⚠ 경고(슬롯 tracking 설정돼야 정상).
    ...
    if status.get("upstream_ok"):
        ...
        upstream = "⚠ 미해소 (`@{upstream}` 없음 — 슬롯 tracking 미설정)"
...
# **만** 한다(자동 채택 없음·추론 금지). 흔한 base 브랜치를 훑어 merge-base
...
# main-참조 해소 커맨드가 제시할 **신규 브랜치명** 파생 규칙.
...
def _phase0_diverge_reason(result) -> str:
    """0단계 record-vs-live FAIL-LOUD 의 판정 근거 한 줄 (surface-only).
...
def _format_slot_era_warning(info: dict | None) -> str | None:
    """슬롯 시대차 info(`_slot_era_info` 반환) → 경고 줄 또는 None.
...
class PmBootstrap:
    ...
    def __init__(
        ...
        *,
        ...
        # board submodule 디렉토리— 명시(hermetic 테스트) 없으면 *호출 시점*
        ...
        # pm_state seam (차수 announce + 인계 dump) — bound slot 의 per-slot pm_state.
        ...
        # areas_file 미지정이면 `_areas_file()`(board_root 추종)로 해소 — board/
        # 분리 후 등록영역 surface(_registered_repos)가 stale 안 보게. 명시
        ...
        # 경로 진입 시에만 동적 로드(multi-PM 모드)·무인자 경로는 안 건드린다.
        self._worktree_pool = worktree_pool
        # board seam — 보호 브랜치 surface(`_repo_protected`)용. 테스트는 mock
        ...
        # run() 진입부에서 명시 multi-PM 모드면 세팅, 무인자면 None
        # 유지(→ `_worktree_cwd` 가 `_auto_slot` 으로 자동해소). board 러너는 무관(REPO 고정).
        self._bound_slot: str | None = None

        # task 세션 식별자 — `--task <이름>` 바인딩 성공 시 run() 진입부에서 세팅.
        # task가 유일한 진입 정체성이며, log/pm_state 귀속을 task 태그(`task:<이름>`)·task 서술 pm_state
        # (`.local/tasks/<이름>/pm_state.md`)로 좁혀 resume-read(차수·남은작업)를 완결한다.
        # None(무-task)이면 모든 task 분기가 no-op → slot dump 100% 불변(회귀 가드).
    ...
    # ── git/pytest 러너 cwd 해소 (worktree) ───────────────────────

    def _worktree_cwd(self, slot: str | None = None) -> str:
        """git/pytest 를 돌릴 작업 디렉토리를 해소한다 (분리된 PM 홈+worktree 모델).

        자기분리토폴로지: 코드/tests=worktree·board/wiki=PM 홈. 분리된 PM
        ...
        해소 순서:
          ...
          - 그것도 없으면(판정 불능 — 모호·엔진 사본 부재) **현 `REPO`**. 등록된 홈은 홈 자신이
            슬롯 행이라 위 층에서 해소되므로, 이 층은 형상 추론이 아니라 dump 가 멈추지 않게
            하는 마지막 트리다(read surface).
    ...
    def _pm_state_display_path(self) -> str:
        """첫-turn 안내에 쓸 pm_state 경로 (task·per-slot).
        ...
        무인자면 `_auto_slot()` 단일 self-host 자동해소(인스턴스 `_areas_file` 추종),
        둘 다 미해소면 미확정 표기다. 슬롯 축은 모듈 `_pm_state_display_path`에 위임한다 —
        slot 키는 pm_handoff 의 read/write 경로와 동형(`<repo>_<N>`).
        """
        if self._task_name is not None:
    ...
    def _default_run_pytest(self) -> tuple[int, str]:
        ...
        # cwd 는 _worktree_cwd 가 해소한다— 분리된 PM 홈엔 tests/ 가 없으므로
        # 회귀는 활성 worktree 슬롯에서 돌아야 한다(미세팅이면 REPO 폴백).
        result = subprocess.run(
    ...
    def _default_run_git(self, args: list[str]) -> tuple[int, str]:
        ...
        # cwd 는 _worktree_cwd 가 해소한다— 분리된 PM 홈엔 코드 git 이 없으므로
        # git dump 는 활성 worktree 슬롯에서 돌아야 한다(미세팅이면 REPO 폴백).
        # 네트워크 계열(fetch/pull)만 timeout— present-but-unresponsive 원격이 세션
    ...
    def _collect_board(self) -> dict:
        ...
        렌즈는 task/슬롯 정체성에 따라 갈린다:
          ...
          - **명시 슬롯 바인딩**(`--repo`/`--slot`·multi-PM·`self._bound_slot` 세팅)이면 **그 슬롯
            정체성**으로 조회한다(`list --repo <repo> --slot <N>` = 현재-사용자 ∩ 그 슬롯).
            무조건 `list --mine`(user·전 슬롯)로 뽑아 "claimed 4 (mine)" 이 `list --repo REPO --slot 3`
            카운트 = 그 세션 뷰와 정합. 라벨 "(slot N)".
          - **무바인딩**(`_bound_slot` None)이면 현행 `--mine`(내 area open + 내 claim·전
            슬롯) 유지. user 미상이면 board 가 전체 open + 내 슬롯 claim 으로 graceful 폴백
            하므로 현행과 사실상 동등. 라벨 "(mine)".
        전체 보드(contention 가시·backlog 확인·타 세션 claim)는 `board list --all` 로 PM 이 명시
        조회한다(무인자 기본은 내 세션 스트림만·타 세션 티켓 정보는 기본 dump 에서 완전 제거).

        반환 `counts` 는 이 렌즈 스코프 값이다.
        라벨 명확화는 빌더(`_format_board_counts_line`)가 `counts_scope` 로 담당한다.

        **done 카운트는 default 뷰(무-`--status`) 비의존**:
        `board.py list`(무-status)가 done 을 접어활성 상태(open/claimed/blocked)만
        ...
        # 세션 기본 뷰: 이 렌즈 조회(`list --repo X --slot N` 또는 `--mine`)가 곧 **내 세션
        # 스트림**(내 세션 생성 open + 내 세션 claim)이다 — board.py 가 타 세션분을 완전 비노출한다.
        # `open_tickets`·`counts` 는 그 스트림 모수 그대로다(`--all` 전량 정렬·접힘 카운트·
        # 타 세션 티켓 정보는 기본 dump 에서 완전 제거·명시 `--all` 몫).
        counts = parse_board_counts(output)
        open_tickets = parse_open_tickets(output)

        # done 전용 재조회 — default 뷰가 done 을 접어위 counts["done"] 이 항상 0
    ...
    def _collect_board_git(self) -> dict | None:
        """board submodule(`.project_manager/board`) 의 HEAD·dirty·ahead/behind 를 수집한다.

        board 는 `ignore=all` git submodule 이라 부모 repo `git status` 가
        board 의 git 상태를 통째로 숨긴다 — multi-PM 에서 board = claim 즉시-sync 공유채널
        이라 board 가 stale 하면 남의 claim 을 놓칠 수 있다(freshness load-bearing).

        `self._board_dir`(`.project_manager/board`) 가 **실 디렉토리가 아니면**(board
    ...
    def _probe_git_freshness(self, label: str, scope_dir: Path) -> dict:
        """한 git 컨텍스트를 fetch origin 후 branch/dirty/ahead·behind 로 측정한다 (pull 없음).
        ...
        # full ref(`--short` 아님) → `refs/heads/` 접두 정확 제거로 순수 브랜치명
    ...
    def _sync_scope(self, label: str, scope_dir: Path, *, is_home: bool = False,
        ...
        """한 git 컨텍스트를 측정하고 clean·ff 면 `git pull --ff-only` 로 자동 동기한다.
    ...
    def _sync_board_submodule(self) -> dict | None:
        """board 서브모듈 fetch + branch 유지 ff-pull.

        board 미분리(`_board_dir` 비-디렉토리)면 None(skip). fetch 상시(fail-soft),
        ...
        if scope["state"] == "동기" and scope["branch"]:
            was_behind = scope["behind"]
            # branch 유지 동기 — detached HEAD 회피. submodule
    ...
    def _collect_freshness(self) -> list[dict]:
        """각 scope 를 fetch → behind/ahead → clean·ff 면 ff-pull 한다.

        미분리면 한 scope 로 접힌다. 반환 목록을 Git 절 freshness surface 에 쓴다.
    ...
    def _bound_session_name(self) -> str | None:
        """차수 유도/본문 dump 귀속용 bound 세션 키 — **log/pm_state 귀속 필터** 전용.

        **task 모드(`--task <이름>`)면 `task:<이름>` 태그**를 반환한다 — handoff write 측이
        task 세션 종료를 `(task:<이름>)` sentinel 태그로 박고 task 서술 pm_state 를 쓰므로
        이 read 측이 같은 키로 자기 태그 entry(차수·본문)를 되읽어 resume 루프를
        닫는다(write↔read 대칭). task 진입은 task 앵커만 사용한다.

        task 무설정이면 슬롯/auto 정체성으로 폴백한다(`_slot_session_name` 반환 불변).
    ...
    def _slot_session_name(self) -> str | None:
        """슬롯/auto 정체성 키(`<repo>_<N>`) — board/lease/대시보드 스코프 전용 (task-무관).

        명시 multi-PM 모드(`_bound_slot`=`work/<repo>_<N>`)면 `work/` 접두를 벗긴 `<repo>_<N>`.
        무인자면 handoff **write 측**(`_resolve_session_worktree_slot`)과 **같은 경로로**
        자동해소한다 — 단일 self-host 무인자 부트스트랩이 그 슬롯(`<repo>_<N>`)으로 바인딩되어,
        handoff 가 무인자로 write 한 자기 태그 entry(+무태그 히스토리)를 **둘 다** 자기 것으로
        되읽는다. 이 write↔read 대칭이 없으면(구: 무인자→None→무태그 read) 단일 self-host 에서
        handoff 가 쓴 `(<repo>_1)` 태그 entry 를 부트스트랩이 버려 차수 유실 stale 침묵
        무력화가 났다(adopter#0 실증 버그). 등록 repo 0개·모호·판정불가 → None.

        **task 와 직교**: board 카운트 렌즈·lease/대시보드 자기-제외는 task 태그가 아니라
        슬롯 정체성으로 가른다(task+슬롯 조합에서 자기 슬롯을 "타 PM" 으로 오표시하는 것 방지). task
        무설정이면 `_bound_session_name` 과 값이 같아 slot 경로 100% 불변이다.
        """
        if self._bound_slot:
    ...
    def _auto_bound_session(self) -> str | None:
        ...
        default-1)를 재사용해 **write↔read 대칭**을 보장한다(`{1,2}`→slot-1 등 default-1
        규칙까지 동일). leases 는 *호출 시점* REPO 기준 재구성(monkeypatch 추종·hermetic·
        `_pm_state_display_path` 동형). 부트스트랩 READ 는 미해소(`SlotResolutionError`)·판정불가를
        **crash 없이** None 으로 접는다 — 미해소를 fail-loud 로 막는 건 handoff WRITE 와 bare
        진입(`main` 의 `_resolve_session_slot`)이고, 여기는 그 뒤의 read-only surface 다.
        """
        try:
            identity = _session_slot_identity(self._areas_file, self._leases_file())
        except SlotResolutionError:
            return None  # 미해소/모호 → None(read surface crash 금지·진입에서 이미 loud).
        except Exception as exc:  # noqa: BLE001 — fail-soft: 판정 실패는 None(read surface).
            if _is_engine_rev_skew(exc):
                raise
            return None
    ...
    def _collect_log_entry(self) -> dict | None:
        ...
        인계 컨텍스트 dump 를 전역 마지막 entry 가 아니라 *자기
        슬롯의 마지막 handoff*(태그 필터·무태그 폴백=slot-1)로 좁혀 자기 컨텍스트를 복원한다
        ...
        if bound_session is None:
            ...
            return entry
        # bound 해소됨(단일 self-host·명시 multi-PM) — 자기 슬롯 handoff entry 우선.
    ...
    def _resolve_pm_state_file(self) -> Path | None:
        """bound slot 의 per-slot pm_state 경로를 해소한다 (read-only).

        명시 주입(`self._pm_state_file`·hermetic 테스트)이 있으면 그대로. 없으면
        `pm_handoff._pm_state_path`(per-slot)를 **동적 로드**해
        ...
        **양성 슬롯 바인딩(`self._bound_slot`)이면 legacy 폴백을 금지**한다 (
        "타 슬롯 최소 유입"): 자기 슬롯 pm_state 가 없을 때 `_pm_state_path` 는
        legacy `wiki/pm_state.md`(slot-1 상태)로 폴백하는데, 그러면 fresh slot-2 가 타 슬롯
        차수·남은작업을 가져와 "fresh=1차" 를 깬다. 해소 경로가 자기 슬롯 디렉토리
        (`.local/slots/<slot>/`) 밖(=legacy 폴백)이면 자기 pm_state 부재로 보고 None 을 반환한다
        (fresh 슬롯·surface 생략·1차 규칙은 `_collect_handoff_context` 층). bound None 은
        ...
        # task 모드 — 연속성 앵커가 slot→task 로 이동한 세션은 task 서술 pm_state
        ...
        # 자기 pm_state 부재로 legacy 폴백된 것 → 타 슬롯 상태 유입 금지·None(fresh).
    ...
    def _collect_handoff_context(self, log_text: str | None = None) -> dict | None:
        """bound slot pm_state 차수(+ log 교차검증) + "남은 작업/사용자발의" 절을 수집한다.
        ...
        - `session_num`: pm_state(`infer_session_num`) 와 log(`parse_last_handoff_session_num`+1)
          ...
          가 해소돼도 log-derived 가 더 크면 **log 우선(max)** + stale 표시(머신 간 미동기).
        ...
        # log-derived 차수 폴백/교차검증 — 자기 슬롯 handoff `PM N차` → N+1.
        # bound 세션 태그 필터로 전역 max 가 아니라 슬롯 max 를 쓴다("두 슬롯 같은 N차").
        ...
        # task 세션(`task:<이름>`)은 fresh 슬롯 배너/1차 강제 대상이 **아니다**.
        ...
        # fresh 슬롯 규칙: 명시 슬롯 바인딩인데 자기 슬롯 차수가 전혀 안 잡히면
        # (pm_state·log 둘 다 미해소) 1차부터 — slot-2+ 는 무태그 기존 로그를 무시하므로 fresh
        # 슬롯이 placeholder(`?`) 대신 슬롯-first(1차)로 announce 된다. bound None 은 현행
    ...
    def _read_log_text(self) -> str | None:
        """log/current.md 원문을 읽는다 (차수 log-폴백/user 연속성 공용·fail-soft None)."""
        if not self._log_file.exists():
            return None
        try:
    ...
    def _handoff_commit_author(self, handoff_header: str) -> str | None:
        """직전 handoff entry 헤더 줄을 담은 commit 의 author email (pickaxe).

        `git log -1 --format=%ae -S<header> -- <log_file>` — 그 헤더 줄을 추가한 commit 을
        pickaxe(-S)로 찾아 author email 을 얻는다. log/current.md 는 REPO가 소유하므로
    ...
    def _collect_user_continuity(self, log_text: str | None) -> str | None:
        """직전 handoff 작성자(commit author) vs 현재 user 비교 → 연속성 1줄.
        ...
        """
        # 자기 슬롯 마지막 handoff 헤더로 좁힌다 — slot-2 부트스트랩이 전역
    ...
    def _collect_dashboard_others(self) -> dict | None:
        """타 슬롯 대시보드 섹션(자기 제외) light dump 데이터를 수집한다.
        ...
        - `wiki/log/dashboard.md` 존재 → `pm_handoff.parse_dashboard_sections`(동적로드·DRY)로
          ...
          **활성(leased) 슬롯과 교집합** 섹션들을
          ...
          (자기 컨텍스트는 pm_state/log 몫). `--done` 으로 release 된 idle 슬롯 섹션은
        ...
        - 어느 쪽도 타 PM 0개 → None(절 생략·graceful).
        """
    ...
    def _build_markdown(
        ...
        # 차수 announce — bound slot pm_state 에서 추론한 `PM <N>차`. 미해소/추론불가는
        ...
        # fresh 슬롯 — 첫 바인딩(pm_state·자기 handoff 둘 다 부재). log/pm_state 섹션의
        ...
        # user 연속성 — 직전 handoff 작성자와 현재 사용자 동일성 1줄(미해소면 생략).
        user_continuity = git.get("user_continuity")
        if user_continuity:
            lines.append(user_continuity)
        lines.append("")

        # Board 섹션 — 카운트 스코프 라벨 명확화. 명시 슬롯 바인딩이면 "(slot N)"
        # (그 슬롯 정체성으로 조회)·무바인딩이면 "(mine)".
        lines.append("### Board")
        ...
        # open 상세 = **내 세션 스트림**만(세션 기본 뷰). `open_tickets` 는 이미 세션 렌즈
        # (`_collect_board`)로 뽑혀 내 세션 생성 open 만 담는다 — 타 세션분(그 외 open 접힘 카운트·타
        # 세션 claim 현황)은 기본 dump 에서 완전 제거(명시 `board.py list --all` 몫). 슬롯
        ...
        # git freshness — 각 scope fetch 후 behind/ahead + clean·ff 자동
        ...
        # 재부착 단서 — 현 브랜치가 직전 handoff worktree 브랜치와 다르면.
        reattach = git.get("reattach")
        if reattach:
            lines.append(f"- {reattach}")
        # board submodule freshness — `ignore=all` 이 부모 `git status` 에서 board
        # git 상태를 숨기므로 별도 1줄로 surface. board 미분리면 생략(graceful skip).
        board_git = git.get("board_git")
        if board_git is not None:
            lines.append(f"- board: {_format_board_git_freshness(board_git)}")
        lines.append("")

        # log/current.md 섹션 — 제목 + **본문 전체** dump (인계 컨텍스트 self-surface).
        ...
        if log_entry:
            ...
            # fresh 슬롯 — 이 슬롯의 이전 handoff 없음. 전역 마지막 entry 유입은 타 슬롯
            # 컨텍스트 오염(MF-2)이라 금지 — "복구할 게 없음"을 명시(스크램블 금지).
            lines.append("- (🆕 첫 바인딩 슬롯 — 이 슬롯의 이전 handoff 없음 · 복구할 컨텍스트 없음)")
        else:
            lines.append("- (log/current.md 없음 또는 entry 파싱 실패)")
        lines.append("")

        # pm_state 인계 surface — bound slot 의 "남은 작업/사용자발의" 절. 부트스트랩만으로
        ...
        # pm_state 는 슬롯별— PM 이 *자기 슬롯* pm_state 를 읽도록 per-slot 경로를
        # 안내한다(pm_handoff 의 read/write 경로와 동형). 미해소면 미확정 표기.
    ...
    def _build_json(
        ...
        return {
            ...
            "git": {
                ...
                # JSON에는 키 자체를 방출하지 않아 기존 slot 스키마를 보존한다.
                **(
                ...
                **(
                ...
                # board submodule freshness— None(board 미분리)이면 생략된 것과
                # 동일 정보량(graceful skip). 있으면 head/dirty/ahead/behind.
                "board_git": git.get("board_git"),
                # git freshness— 각 scope 의 fetch/behind·ahead/pull 결과 목록
                # (`board_sync` 하위 포함). None/[] 면 freshness 미수집(빌더 직접 호출 등).
                "freshness": git.get("freshness"),
                # 재부착 단서— 브랜치 불일치 경고 문자열 또는 None.
                "reattach": git.get("reattach"),
                # user 연속성 — 직전 handoff 작성자 vs 현재 user 1줄 또는 None.
                "user_continuity": git.get("user_continuity"),
            },
            "log_last_entry": log_entry,
            # 다른 활성 PM (수정형 대시보드) — 자기 제외 타 슬롯 섹션 light
            # dump({"mode":"dashboard"|"lease","others":[…]}) 또는 None(타 PM 0개).
    ...
    def run(
        ...
        *,
        ...
        repo:        multi-PM 모드— 주면 worktree 슬롯 alloc/bind + identity surface
                     를 *추가* 출력한다. 무인자면 None — 현행 동작 100% 보존
                     (alloc/bind 경로 미진입).
        slot:        multi-PM lean 모드— `--repo` 와 함께 주면 `alloc` 대신
        ...
        # 세션 정체성(`_bound_slot`)을 **모든 수집보다 먼저** 확정한다. 이후 모든
        ...
        #   - 무인자: alloc 미진입 → `_bound_slot` None → `_worktree_cwd` 가 `_auto_slot()` 로
        ...
        # ── 0단계 검증 — dump/alloc 을 뿌리기 *전에* "내가 올바른
        # 슬롯/위치를 쓰고 있나"를 기계로 검증한다. 실패 시 **부분 dump 도 금지**(dump 가 뜨면 PM 이
        # 그것을 세션의 진실로 믿는다) — preflight 가 비-0 을 돌려주면 여기서 alloc/수집/dump
        # 이전에 즉시 중단한다. 엔진 앵커(무조건)+슬롯 실재·점유·기록정합(lean 조건)+main-참조 거부.
        # 무인자(repo None)/alloc(slot None)는 슬롯 검사 자연 no-op·앵커만 무조건(결정 2).
        # alloc 앞단에 둬 앵커 거부 시 신규 lease 잔존을 예방한다.
        preflight_rc = self._phase0_preflight(repo, slot)
        if preflight_rc != 0:
            return preflight_rc

        # ── task 바인딩 — 0단계 통과 *후* 진입. `--task <이름>`
        ...
        if task is not None:
            ...
            # task 정체성을 인스턴스에 확정한다 — 이후 모든 수집(log_entry/handoff_ctx/
            ...
            # ── 진입 검증 — task 보유 슬롯 **전수** 0단계 검증. 슬롯별
        ...
        # alloc 로 갓 잡은 **신규 lease 를 실패에 안전하게** 만든다. alloc 을 앞단에 둔
        # 건 cwd/정체성 정확성 때문이라 유지하되, 이후 fail-fast 수집(board/pytest/git — parse
        ...
        # except 는 예외/SystemExit 에만 발동한다. lean/무인자는 이 경로로 신규 lease 를 안 잡으므로
        # cleanup 대상 아님(`alloc_identity is not None` 가드).
        try:
            # git freshness — 부트스트랩 *첫 단계*로 fetch + clean·ff 자동 ff-pull
            ...
            # board submodule freshness— HEAD·dirty·ahead/behind. board 미분리면
            ...
            if output_json:
                ...
                if task_info is not None:
                    # 보유 슬롯 전수 열거 행렬 — task-only 진입에서 자동 수령한 집합.
                    task_info["slot_set"] = self._task_slot_set_rows(
                        task_info["name"], leases=task_slot_leases
                    )
                    data["task"] = task_info  # task identity(신규/resume·prefix 상태).
                _write_machine_line(json.dumps(data, ensure_ascii=False, indent=2))
            else:
                ...
                # 커맨드 카드 — identity surface 뒤. 이 세션이 쓸 전 커맨드를 정체성
                # (`--repo <repo> --slot <N>`) 채운 완성형으로 dump("--help 자체를 안
                # 가게"). lean 은 정체성 채운 형태·미해소(identity None/session 부재)는 정체성
                # 인자 없는 형태로 분기. 렌더 실패는 fail-soft(카드 절 생략·위 dump 는 유지).
                card = self._safe_command_card(identity)
                if card:
                    print()
                    print(card)
                # task identity surface — 신규/resume·prefix 상태(기본 없음)·
            ...
            # blocking lint — dump-then-warn(abort-before-dump 제거). 위에서 이미
    ...
    # +main-참조 거부(보호브랜치/origin-추적). 판정 근거는 전부 기계(장부·git·compare 프리미티브)이고 로직 중복은 없다 —
    # 엔진 앵커는 board 가드를, 기록 vs live 는 worktree_pool compare를 **소비만**한다.

    def _phase0_preflight(self, repo: str | None, slot: int | None) -> int:
        """0단계 — dump/alloc 이전 위치·소유·상태 검증. 0=통과·비-0=거부(FAIL-LOUD).

        검사:
          1. **엔진 앵커** (무조건·cwd 무관) — REPO(엔진 파일 위치)가 PM 홈이 아니라 worktree
             사본이면 거부 (`board._pm_home_worktree_misanchor` 소비·부트스트랩은 현행
             이 클래스에 무방비였다).
          2~5 **슬롯 검사** — 슬롯이 명시된 lean 경로(`--repo --slot`·무인자 자동해소 포함)만 돈다.
             무인자(repo None)·alloc(slot None)는 슬롯이 없어 자연 no-op(결정 2).
            2. **작업공간 실재** — 장부 lease 도 없고 폴더도 없으면 거부(phantom 슬롯 바인딩 방지).
            3. **타 점유자** — 다른 세션이 그 슬롯을 leased 로 보유하면 거부(readonly 예외).
            4. **보호브랜치/origin-추적** — main-참조(보호브랜치 직접 checkout 또는 upstream 설정)면
               **거부**(부분 dump 금지·readonly 예외·홈 예외).
            5. **기록 vs live** — `compare_slot_git` 소비(불일치=FAIL-LOUD·미기록=loud+질의 훅·홈 예외).

        4·5 는 **작업 슬롯** 전제 검사라 PM 홈 자신(`HOME_SLOT` 행)엔 걸지 않는다
        (`_phase0_is_home_slot`). 2·3(실재·점유)은 홈에도 그대로 적용된다.
        ...
        if self._reject_worktree_copy_anchor():
            return 1
        # 1b. 보호목록 sidecar drift-only reconcile — *다른 clone/사용자*의
        ...
        # 2~5 슬롯 검사 — lean(명시 슬롯)만. 무인자/alloc 은 슬롯이 없어 자연 no-op(결정 2).
        ...
        if lease is None and not self._phase0_slot_folder_exists(wp, slot_id):
            ...
            return 1
        # 2b. 불완전 생성(creating) — **세션 동일 여부·role 무관** 차단(별도 조건). readonly 든
        #     아니든 반쯤 만든 슬롯은 못 쓴다. bind_slot 이 기존 엔트리를 무조건 leased 로 덮어
        ...
        if self._phase0_incomplete_create(lease):
            ...
            return 1
        # 2c. readonly 공유 슬롯(should-fix)은 **바인딩(점유) 대상이 아니다** — `/pm-bootstrap
        #     --slot N` 오지정 방어. readonly 는 무소유 공유 자산(session/pid 없음·배타 대여 없음)이고,
        #     0단계 carve-out이 readonly 를 *조회 지칭*엔 허용하지만 bind 는 *점유*라 의미가 다르다.
        ...
        if self._phase0_is_readonly(lease):
            ...
            return 1
        # 3. 타 점유자 — readonly 는 2c 에서 이미 거부돼 여기 도달 시 항상 work 슬롯.
        ...
        if holder is not None:
            ...
            return 1
        # 4. main-참조(보호브랜치 직접 checkout / origin-추적 upstream) = 진입 거부.
    ...
    def _reject_worktree_copy_anchor(self) -> bool:
        """엔진 앵커(REPO=엔진 파일 위치)가 PM 홈이 아니라 worktree 사본이면 거부.

        board mutation 의 PM-홈 강제와 **같은 클래스** — `board._pm_home_worktree_misanchor
        (REPO)` 를 소비한다(판정 로직 재구현 금지). 부트스트랩은 현행 이 클래스에 무방비였다: worktree
        (코드 전용) 트리에서 부트스트랩하면 그 트리를 세션 진실로 dump 한다. **cwd 는 판정에 불참**
        앵커는 REPO 만. board 직접 import 금지(touches 격리)라 주입/동적로드 board 의 헬퍼를
        `getattr` 로 쓴다(DI 보존·`_protected_warning` 동형) — board/헬퍼 부재·예외는 fail-soft(검사
        생략·standalone 무영향·오탐 0). True=거부(REPO 가 PM 홈 등록 worktree)."""
        board_mod = self._board or _load_board()
        ...
        print(
            ...
            f"PM 홈이 소유합니다. 이대로면 이 worktree 를 세션 진실로 잘못 dump 합니다.\n"
    ...
    def _phase0_is_readonly(self, lease) -> bool:
        """그 슬롯이 readonly 공유 자산(`role="readonly"`)인가 — 0단계 **바인딩 거부** 판별.
        ...
        `ReadonlySlotNotLeasable` 의 user-facing 짝). 0단계 carve-out(identity_args)이 readonly 를
        *조회 지칭*엔 허용하지만 bind 는 *점유*라 의미가 다르다. 판별 축은 canonical `lease.role`
        additive 1급 필드로 승격) — 구 장부(role 부재)는 `from_dict` 가 "work" 로 read 하므로 항상
        non-readonly(회귀 0·하위호환). **`lease.extra` 가 아니라 `lease.role` 을 읽는다**
    ...
    def _phase0_is_home_slot(self, wp, slot_id: str) -> bool:
        """그 슬롯이 PM 홈 자신(`worktree_pool.HOME_SLOT`)인가 — **작업 슬롯 전제** 검사의 배제 판별.

        main-참조 거부(4)와 기록↔live 정합(5)은 "작업 슬롯에서 보호 브랜치에 커밋" 을 막는 검사다.
        PM 홈은 작업 슬롯이 아니라 board/wiki 가 사는 트리이고, 그 브랜치가 `main`/`master` 인 것이
        정상 채택자 형상이라 전제가 성립하지 않는다(홈의 보호는 push 보호 훅이 한다). 판별은 풀이
        이름 붙인 상수와의 값 비교 하나뿐 — 형상 추론이 아니다. 상수 부재 풀(구버전·mock)은 배제
        자체가 성립하지 않아 항상 False 이므로 검사가 종전대로 돈다(과배제 0). 상수 부재와 슬롯 값
        `None` 을 같은 sentinel 로 접으면 미노출 풀에서 `slot_id=None` 이 홈으로 오인돼 두 검사가
        통째로 배제되므로, 상수 부재를 먼저 분리한다."""
        home_slot = getattr(wp, "HOME_SLOT", None)
        return home_slot is not None and slot_id == home_slot

    def _phase0_incomplete_create(self, lease) -> bool:
        """그 슬롯이 불완전 생성(`state="creating"`·provisional/중단 마커)인가 — 0단계 차단 대상.
    ...
    def _phase0_other_holder(self, lease, session: str) -> str | None:
        """그 슬롯을 **다른 세션**이 leased 로 점유 중이면 그 세션명, 아니면 None.
    ...
    def _phase0_protected_reject(self, wp, repo: str, slot_id: str, session: str, lease) -> int:
        """슬롯 live 상태가 main-참조(보호브랜치 직접 checkout 또는 origin-추적 upstream)면 **진입 거부**.

        **거부**한다(BREAKING). 근거:
        )은 신규 파생 경로에만 upstream 자동설정을 억제하므로 **main 직접 checkout·수동 tracking
        은 무방비** — 커밋이 다 된 뒤 pre-push 훅이 마지막에 잡는 구멍("오염 커밋")을
        ...
        **홈 예외**: PM 홈 자신(`HOME_SLOT` 행)은 이 검사의 전제인 작업 슬롯이 아니다 —
        board/wiki 트리가 `main`/`master` 인 것이 정상 채택자 형상이고, 홈에서의 보호는 이 게이트가
        아니라 push 보호 훅이 한다. 배제하지 않으면 채택자의 첫 부트스트랩이 곧바로 막힌다.

        **readonly 예외**: readonly 공유 슬롯(role="readonly")은 detached(브랜치 없음·upstream
        없음)라 두 축에 자연 미해당하지만, role 로도 명시 carve-out 한다 — main-참조 역할을 이전받는
        슬롯이 곧 readonly 이므로 그 자체가 거부되면 자기충돌. (bind flow 에선 2c 가
        ...
        브랜치 전환)을 **실값**으로 싣는다."""
        # 홈 예외 — PM 홈은 작업 슬롯이 아니라 이 검사의 전제(작업 슬롯 커밋)가 성립하지 않는다.
        if self._phase0_is_home_slot(wp, slot_id):
            return 0
        # readonly 예외 — 무브랜치 공유 자산은 main-참조 판정 대상이 아니다(역할 이전 대상 자기보호).
        if self._phase0_is_readonly(lease):
            return 0
    ...
    def _phase0_main_reference_reason(self, wp, repo: str, slot_id: str) -> str | None:
        """슬롯이 main-참조 상태인지 판정 — 사유 문구(거부용) or None(통과) (두 축).

        두 축(어느 하나라도 해당 시 거부) — `main`+`origin/main` 슬롯이다:
          1. **보호브랜치 직접 checkout** — 슬롯 HEAD 브랜치가 보호목록(main 등)이면. `_protected_warning`
             재사용(보호목록=`board._repo_protected`).
          2. **보호브랜치 원격 origin-추적** — `@{upstream}` 이 보호 브랜치 원격(`origin/main` 등)을 가리키면.
             ...
             게 정상(미해소=경고)이라 그건 통과시킨다. 보호브랜치 원격을 추적할 때만 main-참조.
    ...
    def _phase0_protected_upstream(self, wp, repo: str, slot_id: str) -> str | None:
        """슬롯 `@{upstream}` 이 **보호 브랜치 원격**(`origin/main` 등)을 추적하면 그 upstream 이름, 아니면 None.

        판정 로직 재구현 금지 — worktree_pool 의 `slot_status`(`_upstream_status` 흡수)를 호출만
        ...
        작업 슬롯이라 제외**(오탐 0 — concern 은 `origin/main` 추적). slot_status 미지원(구 풀)·None·
    ...
    def _remedy_branch_name(self, wp, repo: str, slot_id: str, preferred: str) -> str | None:
        """main-참조 해소 커맨드가 제시할 **안전한 신규 브랜치명** (소비처 2곳 공용).

        해소 커맨드가 실제로 실행 가능한 값만 돌려준다 — 세션/task 명을 그대로 보간하면 그 이름이
        보호브랜치이거나 이미 존재하는 브랜치일 때 자기모순 안내가 된다.
    ...
    def _remedy_branch_ref_ok(self, wp, slot_id: str, branch: str) -> bool:
        """후보가 **`switch` 가 받아들일** 브랜치명인가 — worktree_pool 판정 재사용 (규칙 중복 0).
    ...
    def _remedy_switch_command(self, wp, repo: str, slot_id: str, preferred: str) -> str:
        """main-참조 해소 커맨드 문자열 — **엔진-매개 단일 커맨드** (소비처 2곳 공용).

        raw `git switch -c <b>` 는 브랜치는 바꾸지만 장부 스냅을 안 남겨서, 사용자가 안내대로
        해소하면 **곧바로** 0단계 "기록↔live diverged" 차단이 뜬다(왕복 2회 강제).
        엔진이 매개하는 전환은 전부 스냅을 재기록하는데 remedy 만 엔진 밖 raw git 이던 비대칭을
        `worktree_pool.py switch`(전환+스냅 재기록 원자)로 닫는다.

        브랜치명은 `_remedy_branch_name`이 산출한다(규칙 중복 구현 0). 후보 소진(None)이면
    ...
    def _slot_branch_exists(self, wp, slot_id: str, branch: str) -> bool:
        """그 슬롯 worktree 에 로컬 브랜치 `branch` 가 이미 있는가 (remedy 후보 충돌 검사).
    ...
    def _phase0_record_vs_live(self, wp, slot_id: str) -> int:
        """기록된 lease.git(기대) vs 슬롯 live 정합 — `compare_slot_git` 소비.

        compare 프리미티브를 **호출만** 한다(compare/판정 로직 재구현 금지). 판정 정책만 여기서:
          ...
          - `unrecorded`(구 슬롯·git 필드 부재) → **차단 아님·loud 표시**("기준점 미기록 — drift 감지
            비활성") + 질의 훅(사용자 `set-base` 질의는 같은 0단계 지점에서 채운다).
        ...
        submodule 만 본다(base=recorded-only breadcrumb). base 불일치 FAIL·"base 대비 N behind"
        판정은 rebase 으로 이월된다(reviewer 추적성).
        `compare_slot_git` 미구현 풀(구버전·mock)·예외는 fail-soft(정합 검사 생략·통과) — 소프트 진단.
        **홈 예외**(main-참조 거부와 같은 축): PM 홈은 세션이 두고 가는 작업 트리가 아니라
        `_phase0_is_home_slot` 이면 검사하지 않는다.
        반환 0=통과·1=FAIL-LOUD 거부."""
        # 홈 예외 — 게이트 4 와 같은 전제(작업 슬롯). 홈은 세션이 두고 가는 작업 트리가 아니라
        # 브랜치·head 가 채택자 평시 운영으로 움직이므로 "두고 간 기록과 다름" 이 외부 개입 신호가
        # 아니다. 4 만 배제하면 홈이 브랜치를 옮긴 뒤 여기서 다시 막힌다.
        if self._phase0_is_home_slot(wp, slot_id):
            return 0
        ...
        if getattr(result, "unrecorded", False):
            # 미기록(구 슬롯) — 차단 아님·loud 표시 + **후보 제시**(자동 채택 없음).
    ...
    def _unrecorded_base_candidates(self, slot_dir: str) -> list[tuple[str, str]]:
        """미기록 슬롯의 base 후보 — `git merge-base HEAD <cand>` 가 해소되는 (branch, sha) 목록.

        **제시용일 뿐 자동 채택 없음**(추론 금지). 흔한 base 브랜치
    ...
    def _unrecorded_base_candidate_line(self, wp, slot_id: str) -> str:
        """미기록 0단계 후보 제시 줄(`\\n  후보: …`) 또는 빈 문자열 (자동 채택 없음).

        슬롯 경로(`wp.slot_path`)를 얻어 `_unrecorded_base_candidates` 로 후보를 계산하고, 있으면
        형식("후보: `origin/main`(merge-base `<sha>`)")으로 포맷한다. 후보 없음·경로 해소
    ...
    def _release_alloc_lease_failsoft(self, slot: str) -> None:
        """alloc 로 갓 잡은 신규 lease 를 best-effort release 한다 (실패 cleanup·원 예외 안 가림).

        부트스트랩이 alloc(앞단) 후 fail-fast 수집(board/pytest/git)에서 abort 하면 이 신규
    ...
    def _bind_task_or_reject(self, task: str) -> dict | None:
        """`--task` 신규/resume 바인딩. 동시 세션(살아있는 다른 pid)이면 None(거부).
        ...
        직교라 이 바인딩은 `.local/slots/` 를 건드리지 않는다.
        """
    ...
    def _build_task_identity_markdown(self, task_info: dict) -> str:
        """task identity surface — 정체성·prefix 상태(기본 없음)·서술 pm_state 포인터.

        prefix 는 이 task 세션의 board prefix 상태를 surface 한다(기본 None → "(없음)") → PM 이
        사용자와 확인. 변경 명령은 `task prefix`. 작업공간(슬롯) 연결은 alloc이
        ...
        lines: list[str] = []
        lines.append("### task identity surface ")
        lines.append(
            f"- 당신은 **task `{name}`** PM · prefix={prefix_display} · 상태={action_label}."
        )
        # loud notice — dead-pid 회수(reclaimed)는 crash 재개가 다수지만, 기록 pid=부트스트랩
        ...
        if slot_set:
            ...
            # 0개 보유 — 진입(검증 no-op) + generic 안내는 여기서만.
    ...
    def _validate_task_slot_set(self, task: str, *, leases: list | None = None) -> int:
        """task 보유 슬롯 **전수** 0단계 검증 — fault 1+ = 진입 차단.
        ...
        lines = [
            ...
            f"진입 차단(부분 dump 금지). **아래 전부를** 해소한 뒤 다시 부트스트랩하세요 "
    ...
    def _task_slot_fault(self, wp, lease, task: str) -> "tuple[str, str, str] | None":
        ...
        if not self._phase0_slot_folder_exists(wp, slot_id):
            return (
                ...
                "외부 삭제/미생성(클래스·엉뚱한 트리 vacuous 진입 방지).",
    ...
    def _task_slot_set_rows(self, task: str, *, leases: list | None = None) -> list[dict]:
        """task 보유 슬롯 전수 열거 행렬 데이터 (fail-soft·부작용 0).
    ...
    def _task_slot_git_status(self, wp, slot_id: str) -> dict:
        """`slot_git_status(slot)`(branch·head·dirty) fail-soft 조회 — 미제공/실패는 {}."""
    ...
    def _append_task_slot_matrix(self, lines: list, task: str, slot_set: list) -> None:
        """보유 슬롯 전수 열거 행렬을 `lines` 에 추가.

        헤더 `작업공간 (task 'X' 보유 N — 전수 검증):` + 슬롯별 `slot · repo · branch · head ·
        기록↔live <mark> · <dirty>` 행(`_render_task_slots`[pm_config]과 같은 ` · ` bullet
        문법)."""
    ...
    def _alloc_and_identity(
        ...
        - `NeedsCreate` (풀 소진) → 사용자 게이트 안내 후 sys.exit(1). **자동
          `git worktree add` 안 함**.
        - 성공 시 cwd=슬롯 경로·branch·등록영역을 dict 로 반환(markdown/JSON 빌더가 소비).
          branch 는 `worktree_pool.current_branch(slot)` live 조회
        ...
        # 을 채운다. 이게 없으면 커맨드 카드가 `session` 키 부재를 미해소로 오판해 `--repo/--slot`
        ...
        return {
            ...
            # 슬롯 상태 surface — upstream + submodule 역할(pin/dev-ahead/
    ...
    def _bind_and_identity(self, repo: str, slot: int) -> dict:
        ...
        - branch 는 `worktree_pool.current_branch(slot_id)` live 조회(git=진실
          ). detached/조회불가/슬롯 폴더 부재는 None → surface 가 "(미지정)".
        ...
        return {
            ...
            "branch": live_branch,
            "others": others,
            # 보호 브랜치 경고 (소프트) — 라이브 브랜치가 그 repo 보호목록에 있으면
    ...
    def _safe_slot_status(self, wp, slot: str) -> dict | None:
        """worktree_pool.slot_status(slot) 를 fail-soft 로 JSON-safe dict 화한다.
    ...
    def _slot_status_block(self, identity: dict | None) -> list[str]:
        """identity 의 슬롯 상태를 `### 슬롯 상태` 서브섹션 줄로 렌더한다 (없으면 빈 리스트).
        ...
        if not lines:
            return []
        return ["", "### 슬롯 상태 (upstream·submodule)"] + lines

    def _protected_warning(self, repo: str, branch: str | None) -> str | None:
        """라이브 브랜치가 그 repo 보호목록(`board._repo_protected`)이면 그 브랜치명 (소프트).
    ...
    def _protected_retry_command(self, repo: str) -> str:
        """보호목록 재실행 안내 커맨드 — `pm_config.protected_retry_command` 소비.
    ...
    def _protected_hook_wired(self, repo: str) -> bool | None:
        """bare `core.hooksPath` 배선 여부 — `pm_config.protected_hook_wired` 소비.

        판정 로직을 재구현하지 않는다(`_load_tool` DRY 관용구 — 복붙 금지
    ...
    def _slot_scope_fetched(self, freshness: list[dict] | None) -> bool | None:
        """freshness 목록에서 현재 슬롯 cwd scope 의 `fetched` 상태.
    ...
    def _slot_era_info(self, repo: str, freshness: list[dict] | None) -> dict | None:
        ...
        폴백 반환 형상은 JSON 계약을 유지하고 formatter가 `repo-default`로 표기한다.
        """
    ...
    def _build_slot_identity_markdown(self, identity: dict) -> str:
        ...
        라이브 브랜치가 보호목록이면 🚫 경고 줄을 정체성 선언 직후 surface 한다
        ...
        lines: list[str] = []
        lines.append("### multi-PM identity surface (lean)")
        lines.append(
        ...
        # 슬롯 시대차 경고 — HEAD 가 base(main) 대비 behind N 이면
        ...
        # 슬롯 상태 — upstream + submodule 역할(drift 경고 vs dev-ahead 정보). 백본
    ...
    def _safe_command_card(self, identity: dict | None) -> str | None:
        ...
        된다 — identity surface·board/git dump 는 이미 나갔으므로
        카드만 조용히 생략한다.
        """
        try:
    ...
    def _build_command_card_markdown(self, identity: dict | None) -> str:
        """이 세션이 쓸 전 커맨드를 정체성 채운 완성형으로 dump 하는 커맨드 카드.

        identity surface 뒤에 코드 생성해, PM 이 --help 왕복 없이 세션 전체를 운영하게 한다
        ("--help 자체를 안 가게"·사용자 지시). 정체성(`--repo <repo> --slot <N>`
        canonical)은 **실값으로 보간**하고,
        ...
        identity: lean(멀티-PM) 모드면 `slot`(`work/<repo>_<N>`)+`repo` 키를 담은 dict → 슬롯
                  ...
                  `slot`/`repo` 부재(미해소/legacy)면 정체성 인자 없는 현행 형태로 분기.
        ...
        아래 1줄 ⚠ 경고로 인접 배치한다(별도 절 금지·인접성이 학습 보장).

        wave 운영(claim·regression·finish·qa·dev 위임·handoff·엔진 갱신)은 **스킬(`/pm-…`) 진입을
        primary** 로 올리고 감싸는 backbone 은 강등한다(boundary=래핑 스킬
        ...
        # (불완전 dict)은 정체성 없는 방어 렌더로 폴백(fail-soft·카드 절 무손상). 게이트 통과 시
        # 번호는 위 slot_num(슬롯 식별자 파생)으로 채운다. lean=` --repo <repo> --slot <N>`·미해소=빈 문자열.
        ...
        def cmd(rec: _CardCmd, comment: str = "", suffix: str = "", prefix: str = "") -> str:
            ...
            커맨드 토큰(도구·서브커맨드·플래그)은 `rec`(공용 정의서) 단일 진실에서 온다 —
            손 문자열 하드코딩 제거. `suffix`/`prefix` = 정체성(`--repo/--slot`·task 명) 등 caller 가
            실값 보간하는 꼬리/머리("정체성은 실값·사용자 입력만 placeholder"; pm_handoff 는
            정체성이 앞에 오므로 `prefix`).
            """
        ...
        # ── 모드별 렌더 — 현재 모드분만 dump(신호 대 잡음·컨텍스트 잠식 방지).
        # task 세션(`--task`)이면 task 커맨드, readonly 공유 슬롯(role=readonly)이면
        # 조회만, 그 외(슬롯)면 현행 슬롯 카드(+ task 모드 발견성 1줄). 안 쓸 커맨드는 안 뿌린다.
        ...
        lines: list[str] = []
        lines.append("### 이 세션 커맨드 카드 (정체성 채움·--help 불요·단일 진실)")
        # 정체성 헤더 — 실값 보간(placeholder 0). 미해소면 정체성 인자 불요 명시.
        if session:
            ...
            lines.append(
                "정체성: 미해소 — 보드/리스 조작은 `--repo <repo> --slot <N>` 명시가 필요하다"
                "(해소 층: env `PM_SESSION_NAME` > 활성 lease 정확히 1개). 장부에 슬롯 행이 "
                f"하나도 없으면 이 홈은 아직 등록 전이라 `{_runtime_skill_entry('pm-update')}` "
                "1회가 등록 repo 1개인 홈을 첫 슬롯 행으로 등록한다."
            )
        # 스킬-우선 운영 pointer — 규칙/why 는 pm_role 이 단일 진실(재설명 금지).
        lines.append(
            "> wave 운영은 스킬로 invoke·backbone 직접호출 금지 → pm_role §스킬 우선 운영 규율"
        )
        # task 모드 발견성 1줄 — 슬롯 카드는 슬롯 커맨드만 뿌리되,
        ...
        # 세션 기본 뷰(내 세션 스트림만·타 세션분 완전 비노출)다. 전체
        ...
        # (카드=pm_role 표기). 숨은전제 ⚠ 는 강등 backbone claim 줄 바로 아래 인접(불변식 2).
        lines.append(
            "# wave 운영 (스킬로 invoke — backbone CLI 엔진은 직접 금지)"
        )
        ...
        # 릴리즈 (직접 — 래핑 스킬 없음) — livegate record 는 release-marked pin.
        # 정체성(`sess`)을 실어 실행가능 형태로 emit — multi-lease 홈에서 정체성 인자 없는 record 는
        # cwd 모호 fail-loud 이므로, 이 세션 슬롯을 명시해 안내 명령이 dead-end 가 아니게 한다
        # (미해소는 `sess`="" → 무인자·현행 형태).
        ...
        # ID·카테고리 유지보수 (드묾·전제 주의) — prefix rename/merge·reid=홈 git clean(
        # reid 추가)·migrate-identity=단일세션. 각 커맨드 줄 바로 아래 1줄 ⚠.
        ...
        lines.append(cmd(
            _C_BOARD_MIGRATE_IDENTITY, "이전 데이터 일회성 backfill",
        ))
        lines.append(
        ...
        # 찾아가기 (부트스트랩 dump 에 없는 것 — 포인터만·정식 서술은 pm_role).
        # 각 항목 "평시 안 읽음·필요할 때만".
        self_tag = f"`({session})`" if session else "`(slot-1=무태그)`"
    ...
    def _codex_card_section() -> str:
        ...
        모든 모드 카드(슬롯·task·readonly) 렌더 끝에 append 되며, 호출이
        `_build_command_card_markdown`(→ `_safe_command_card` try/except) 안이라 감지/문자열화가
        터져도 fail-soft(절 생략·부트스트랩 무손상). 비-codex 하네스는
    ...
    def _task_command_card_lines(
        ...
        """task 모드 커맨드 카드 — task 세션이 쓸 task-스코프 커맨드만 dump.
        ...
        """
        # task 정체성 suffix/prefix — 실값 보간. `<name>` 은 실 task 명(placeholder 아님).
        ti = f" --task {task_name}"
        lines: list[str] = []
        lines.append(
            "### 이 세션 커맨드 카드 (task 모드·정체성 채움·--help 불요·단일 진실)"
        )
        lines.append(
        ...
        lines.append("# 내 작업 보기 (read-only 조회·직접 — 자기 공간 우선)")
        # task 스코프 렌즈(`list --task <이름>`) — 이 task 명의
        ...
        lines.append(cmd(
            _C_BOARD_LIST_MINE,
            "내 것 전 슬롯·전 task(user-wide·직교 렌즈)·기본 user 조회",
        ))
        lines.append("")

        # task 작업공간 — task 명의로 idle 슬롯 대여/반납. 자동 생성 안 함(풀 소진=사용자 게이트).
        lines.append("# task 작업공간 (task 명의 슬롯 대여/반납)")
        lines.append(cmd(
            _C_PC_ALLOC, "idle 최소번호 슬롯 대여(자동 생성 안 함·풀 소진 시 add 요청)", suffix=ti,
        ))
        lines.append(cmd(
            _C_PC_RELEASE, "작업완료 반납(소유검사 이 task 명의 슬롯만)", suffix=ti,
        ))
        lines.append("")

        # task 정체성 — prefix 지정/해제 + task 종료(일괄 반납·아카이브). name 은 위치인자(보간).
        lines.append("# task 정체성 (prefix·종료)")
        lines.append(cmd(
        ...
        # wave 운영 (스킬 primary) — 강등 backbone 은 task 정체성(`--task`) 보간. 슬롯 카드와
        # 동형이나 정체성 축만 slot→task. 숨은전제 ⚠ 는 claim 강등 줄 바로 아래 인접(불변식 2).
        lines.append("# wave 운영 (스킬로 invoke — backbone CLI 엔진은 직접 금지)")
        lines.append(skill("/pm-wave-claim T-NNNN", "ticket claim — DoD 자족 검증 + claim"))
        lines.append(engine(_C_BOARD_SHOW))
        lines.append(engine(_C_BOARD_LINT))
        ...
        # task 세션은 `--task` 를 실어야 slot 축으로 오해소하지 않는다(다른 task backbone 과 동형).
        lines.append(engine(
            _C_TICKET_FINISH,
            "스킬이 부르는 내부 엔진·직접 금지 — 내부서 board.py complete 수행",
            suffix=ti,
    ...
    def _readonly_command_card_lines(
        ...
        """readonly 공유 슬롯(role=readonly) 커맨드 카드 — 조회/갱신만 dump.
    ...
    def _build_identity_markdown(self, identity: dict) -> str:
        ...
        lines: list[str] = []
        lines.append("### multi-PM identity surface")
        lines.append(
...
def resolve_repo_arg(positional: str | None, flag: str | None) -> str | None:
    """positional `repo` 와 `--repo` alias 를 단일 repo 값으로 정합한다.
    ...
    값을, 둘 다 None(미지정)이면 None 을 흘려 무인자 자동바인딩경로를 보존한다.
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ── multi-PM 모드 (0011) — 무인자면 자동해소·현행 보존 ──
...
def _main(argv: list[str] | None = None) -> int:
    ...
    # auto-resolve **앞**에 둔다: 기준은 파싱 직후의 사용자 명시값(`args.repo is None`
    ...
    # guarded 자동바인딩 — `--repo`/`--slot` 둘 다 없는 bare 무인자 호출에서,
    ...
    # task 는 슬롯 0개로 시작 가능해야 하므로 슬롯 자동해소를 태우지 않는다(auto-task
    ...
    if args.repo is None and args.slot is None and getattr(args, "task", None) is None:
        ...
        if auto is not None:
            ...
            print(f"슬롯 자동 해소: repo={args.repo} · slot={args.slot}",
                  file=sys.stderr)
    # 정체성 인자 fail-loud 검증 — bare `--slot`(--repo 없이)·slot<1 은
    ...
    # task 명 예약 패턴 거부 — `<등록 repo>_<N>` 슬롯 세션명과 시각적·기계적 충돌 방지.
