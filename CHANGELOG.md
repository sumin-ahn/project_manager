...
- **BREAKING — 운영 단위가 티켓에서 묶음(클러스터)으로 바뀐다.** 설계·리뷰·fix 는 묶음당 1회, 개발만
  ...
  크기 1 로 접히므로 파일 마이그레이션이 없다. 발행은 활성 묶음이 하나면 거기에 귀속시키고 없으면
  그 티켓 이름의 크기 1 장부를 함께 만들며, 그 사실을 stderr 1줄로 고지한다. 자동 생성된 크기 1
  장부는 통합·묶음 브랜치를 선언하지 않으므로 종결의 재배치·머지 단계가 무대상으로 건너뛴다
  (비차단 · `board.py cluster new` 로 선언하면 그 장부는 흡수된다).
