...
- **PM 홈 push 는 회귀를 요구하지 않는다 — 회귀 게이트는 `tests/` 가 있는 트리(코드 repo)에만
  ...
  **lint 게이트는 두 형상 모두 그대로 push 를 막는다** — 대상이 그 트리 자신의 board/wiki 라 코드
  ...
  없는 트리에서 **pytest 를 띄우기 전에 거부**한다 — test_cmd 가 `tests/` 를 지목하는데 그 트리에
  `tests/` 가 없는 경우이며, 측정도 기록도 하지 않고 처방(`--cwd <코드 트리>` / `--task <이름>`)을
  낸다. 스위트가 `tests/` 밖인 커스텀 test_cmd 는 무영향이다. rc5(수집 0) 진단도 더 이상 세션/
  lease 설정을 지목하지 않는다(회귀 cwd 가 그것을 보지 않으므로 거짓 처방이었다). (2) 훅 세대
  판정이 `lint --gate` 진입에도 붙는다 — 회귀 게이트가 안 도는 트리에서도 구버전 훅이 있으면 같은
  `board.py init` 처방으로 push 가 막힌다(보고 전용 `lint` 는 무변경).
