...
| `additional_reviewer.py` | 🟡 **선택 · 유료 호출** | 추가 리뷰어(additional reviewer) 어댑터(ADR-0004). 채널 스위치는 없고, 실행에는 `local.conf` 대상 튜플(`additional_reviewer.harness`/`.model`/`.reasoning`)이 필요하다. 호출마다 비용 승인을 다시 받지 않는다. 없어도 내부 code-reviewer 로 완결. |
