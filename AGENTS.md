...
세션 시작 필독 셋은 이미 로드된 진입문서, 현재 정체성의 `pm_state`, `/pm-bootstrap`(opencode) / `$pm-bootstrap`(codex) dump 한 번뿐이다.
...
3. **`/pm-bootstrap`(opencode) / `$pm-bootstrap`(codex) dump 한 번** — board·git·차수·직전 handoff 본문·남은 작업을 한꺼번에
...
| PM-workflow 스킬 | canonical `SKILL.md` 단일 소비. `/pm-bootstrap`(opencode) / `$pm-bootstrap`(codex) 같은 진입 표기로 호출하며 스킬 스캔 비활성화 금지. |
