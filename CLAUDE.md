...
> Claude Code 세션이 시작될 때 자동 로드되는 진입점. **새 세션이라면 먼저 이걸 읽고 → `project_wiki/board.md` 를 확인하라.**
...
1. **상황 파악** — 다음 3 파일을 순서대로 본다:
   - [`project_wiki/board.md`](project_wiki/board.md) — 지금 무슨 ticket 잡을 수 있나?
   - [`project_wiki/status.md`](project_wiki/status.md) — 어디까지 됐나? (모듈 매트릭스 + 외부 의존성)
   - [`project_wiki/architecture.md`](project_wiki/architecture.md) — Layer / 파일 / 의존성 / 계약
   결정 근거는 [`project_wiki/decisions/`](project_wiki/decisions/).
...
3. **Ticket 잡기** — 외부 의존이 없고, 다른 세션이 이미 claim 하지 않은 것을 고른다:
   ```bash
   {{PY}} tools/board.py list --status open
   {{PY}} tools/board.py show T-NNNN
   {{PY}} tools/board.py claim T-NNNN --session session-B
   ```
...
```bash
{{PY}} tools/board.py complete T-NNNN --tests-pass     # 또는
{{PY}} tools/board.py block T-NNNN --reason "..."
```

추가로:
- `project_wiki/status.md` 의 해당 모듈 행 갱신
- `project_wiki/log.md` 에 한 entry append
...
```bash
# 전체 테스트 (현재 테스트 수는 project_wiki/status.md 가 단일 진실)
{{TEST_CMD}}

# 보드 조작
{{PY}} tools/board.py list
{{PY}} tools/board.py show T-NNNN
{{PY}} tools/board.py claim T-NNNN --session <name>
{{PY}} tools/board.py complete T-NNNN --tests-pass
{{PY}} tools/board.py new "title" --touches a.py,b.py --tag phase-1
{{PY}} tools/board.py lint     # 의존성·thin-ticket 일관성 검사
```
...
| 경로 | 의미 |
|---|---|
| `tools/board.py` | Ticket 시스템 CLI |
| `project_wiki/` | 비-코드 산출물 (작업 / 결정 / 사양 / 상태 / 아키텍처) |
...
- 모르는 결정이 필요하다 → ADR 작성 후 진행 ([`project_wiki/decisions/README.md`](project_wiki/decisions/README.md) 의 작성 절차).
