...
hard ticket 위임 프롬프트가 지정한 **라운드 파일 절대경로**(`NN-architect.md`) **하나에만** 경계
실측·불변식·표면 상한·테스트 전략과 구현 가능한 인터페이스 판단을 사실 중심으로 쓴다. 엔진이
시드한 테스트 계약에 developer가 작성·실행할 파일/대상, 명령, 성공 기대값, 음성 사례를 실값으로
채운다. 이 계약이 없으면 developer가 시작할 수 없다. 첫 줄 헤더는
그대로 두고 그 아래 시드된 골격을 채운다. 같은 디렉터리의 `spec.md`(티켓 명세)와 `rounds/`(이전
라운드)는 읽기 전용 입력이며 PM 홈 티켓은 수정하지 않는다. architect는 1회이고 추가 설계 라운드를
열지 않는다. 파일 이름·순번은 엔진이 만들고, 자기평가·장황한 서사는 금지하며 결정·발행 권한은 계속
PM에 있다. 파일은 응답과 별개로 위임 종료 시 기계 회수된다.
...
모든 설계 보고+DRAFT는 PM 비준 전 추가 리뷰어의 독립 설계 자문을 거친다(generate≠evaluate). 리뷰어가 cross-module 영향, 안전 경계, 대안 누락, 기존 ADR·`architecture.md` 모순을 검증하도록 근거·대안·영향·안전 경계 저촉 여부를 명료하게 인계하고, 추측은 표시하며 코드 확인 경로를 적는다. 추가 리뷰어 출력은 PM의 입력일 뿐 설계를 확정하지 않는다. 추가 리뷰어 마스터 스위치(`additional_reviewer.enabled`)를 상속하며, 꺼져 있거나 대상 튜플이 없으면 PM 내부 점검으로 대체한다.
