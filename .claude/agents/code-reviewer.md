...
부여한다. reviewer는 disposition을 쓰거나 설계·지원·권한을 확정하지 않는다. severity와 무관하게
모든 finding은 엔진 골격의 코드 위치·오류 거동·수정 설계·추가 회귀 테스트·실행 명령·기대값을
모두 구체값으로 채운다. 빠진 계약은 fix 입력으로 회수되지 않는다.
