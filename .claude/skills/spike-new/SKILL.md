...
> ⚠️ **혼자 다 쓰고 끝내지 않는다.** 실측은 먼저 파악해 보고하고, 옵션·결정은 사용자와 한 절씩 합의하며 채운다.
> spike 는 **sealed 후 IMMUTABLE** — 생성 시 `status: draft`(편집·세션무관 resume), 합의+사용자 사인오프 시 `sealed (<date>)`. sealed 후 개정은 새 날짜 파일.
...
0. **주제 · scope 합의 (먼저!)** — *무엇을 설계하나* + *무엇은 범위 밖인가* 를 사용자와 한두 문장으로 확정한다.
   ...
   - 사용자가 준 한 줄이 추상적이면("X 를 유기적으로") scope 를 좁혀 되묻는다: 무엇이 핵심 산출(설계 대상)이고 무엇이 증상/곁가지(범위 밖)인지.
1. **파일 생성 + frontmatter** — 합의된 주제를 `title` 에. 아래 "파일 만들기" 절차.
2. **실측 파악 → 보고** — 코드/데이터에서 현황을 직접 확인하고 사용자에게 보고한다. 여기까진 Claude 가 진행해도 된다 (사실 확인이라 합의 불필요).
   - **주제 고정 가드:** 실측 중 나온 곁가지(개별 버그·죽은 코드 등)는 열린질문이나 별도 메모로 빼고 **주제로 돌아온다.** 곁가지를 옵션 갈림길로 승격시키지 않는다.
3. **사용자와 설계** — 옵션부터는 한 절씩:
...
4. **마무리 + seal** — 모든 절이 합의되면 파일 저장 + 이번 spike 산출만 git add. 끝낼 때 **PM 수렴 권장안 블록**을 출력한다(아래) — PM(orchestrator)이 raw 초안을 받아 발행하도록.
   - **seal 단계:** 설계 절 전부 합의 + DRAFT 완비 + **사용자 사인오프** 확인 시에만 frontmatter `status:` 를 `draft → sealed (<date>)` 로 전환한다. **그 전엔 draft 로 둔다 — 혼자 봉인 금지(사인오프가 seal 트리거).** sealed 부터 immutable·인용 가능 / draft 는 편집 가능. ADR/ticket 발행 입력은 **sealed spike 만**(draft 는 PM 수렴 입력이 아님).
...
```
생성 후: 파일 Read → frontmatter(`title`·`created`·`session`·`related`) 채우고 상단 사용법 주석 블록 삭제. **`status:` 는 `draft` 로 생성한다**(편집 가능·세션무관) — 합의+사용자 사인오프 시 마무리에서 `sealed (<date>)` 로 전환.
...
설계가 끝나도 이 스킬은 **raw/spikes 파일 박제까지만**. ADR/ticket 의 실제 발행(번호 부여, `decisions/`·`tickets/` 파일 생성, `board.py`)은 **PM(orchestrator)이 raw 초안을 참고해 진행**한다 — spike 는 PM 이 보는 위치(decisions/·board)를 안 건드리고, raw 안 DRAFT 가 PM 발행의 입력이다. 사용자 게이트가 걸리는 결정(비가역·비용·외부송신·안전)은 PM 이 사용자 비준을 거쳐 발행.
...
**PM 수렴 권장안 블록** — 마지막 응답에 출력 (PM 이 받아 수렴·발행):
```
...
- **raw/spikes = board 밖** — 자유 파일명. spike 는 `status: draft` 동안 편집 가능·세션무관 resume, **sealed (<date>) 부터 IMMUTABLE** — sealed 후 개정은 새 날짜 파일(vN 누적). immutability 는 생성이 아니라 seal 시점 바인딩.
...
- `.project_manager/wiki/raw/README.md` — `raw/` immutable 컨벤션 + spike draft→sealed 예외
