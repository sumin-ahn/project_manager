...
## 설계 단계 (`design: required` 티켓만)

설계 결함이 코드 리뷰 라운드로 전가되는 것을 막는 단계다. 엔진은 **frontmatter 값과 설계 절 존재만** 판정하고, 설계 내용의 옳고 그름은 PM·리뷰어가 본다. **면제 값은 없다** — 값 집합은 `required | done | n/a` 세 형식뿐이고 폐지된 면제 표기는 발행·`design` 기입 양쪽에서 거부된다.

| `design:` 값 | 의미 | promote / claim |
| --- | --- | --- |
| `required` | 설계 미완 (발행 기본값: `--estimate large`) | 차단 |
| `done` | 설계 절 완성 + 점검 라운드 회수 (PM 기입) | 통과 |
