...
- [ ] **표준 섹션** — 목표 / 인터페이스 / 결정 / 완료 조건 / 참고 / 메모(6절). lint는 **목표·완료 조건·참고 3개만** 누락 차단(`_REQUIRED_SECTIONS`)하므로 인터페이스·결정·메모는 PM이 채운다. `<...>` placeholder도 0개여야 한다(lint 차단).
...
- board.py lint 자동 검증은 placeholder·표준 섹션 중 목표/완료 조건/참고·순환 의존이다. DoD verify-able·self-containment·게이트 분류는 PM이 수동 검증한다.
