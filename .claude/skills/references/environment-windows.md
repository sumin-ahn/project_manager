...
이 canonical 문서와 `environment-posix.md`는 [[T-0681]]에서 Claude/OpenCode 모델 채널의
`.claude/skills/references/`, Codex 모델 채널의 `.agents/skills/references/`, OpenCode 사람 채널의
`.opencode/references/`에 파생 복사한다. 특히 평면 `.opencode/command/<skill>.md`의
`../references/environment-*.md`는 `.opencode/references/environment-*.md`로 해소되어야 한다.
T-0679는 canonical 두 문서만 출하하며 템플릿 파생 사본은 만들지 않는다.
