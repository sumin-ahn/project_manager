---
name: pm-wave-finish
description: "wave 안 ticket 완료 부기 — ticket_finish.py wrapper + 회귀 측정 + status.md 스칼라 + log.md skeleton + board complete + git stage. 모듈 행 비고·log.md 서술·git commit 은 PM 손. Triggers: 'T-NNNN 완료', 'ticket 정리', 'finish', 'pm-wave-finish'."
---
...
3. **log.md complete entry skeleton append** — `## [YYYY-MM-DD] complete | T-NNNN — <title>` 형식. 본문 = `<PM: 무엇을·왜>` placeholder.
...
2. **log.md complete entry 본문 서술** — skeleton 의 `<PM: 무엇을·왜>` 를 실제 내용으로:
   - 변경 파일 목록
   - 단위 테스트 수·증가량
