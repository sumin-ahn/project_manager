...
skill 호출 *전* PM 세션은 이미 다음 5개를 읽어야 한다 (pm_role.md §부트스트랩):
...
- **log.md 마지막 entry**: date·type·title. type=`handoff` 면 직전 PM 종료 정합 · `complete` 면 wave 진행 중일 수 있음.
...
3. **다음 옵션 N개** — pm_role.md "남은 작업 전체 그림" 의 우선순위 인용.
