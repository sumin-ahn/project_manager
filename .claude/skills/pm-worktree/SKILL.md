...
> 장부 원자 갱신)하고, readonly 공유 슬롯을 released 최신으로 **갱신**(`refresh`)하고, 슬롯 lease git 스냅을 **명시 재동기**(`record` — 부트스트랩 0단계 diverged 를 사용자가 정당 판단했을 때)하며, 슬롯 브랜치를 **전환+스냅 재기록 원자**로 옮긴다(`switch` — 0단계 main-참조 해소). backbone =
> `.project_manager/tools/worktree_pool.py`(`dev`/`sync`/`set-base`/`status`/`rebase`/`refresh`/`record`/`switch`). 비즈니스
> 로직 0 — 엔진 CLI 호출 thin wrapper.
...
- **기준점(base) 지정** — 부트스트랩 0단계가 "기준점 미기록 — drift 감지 비활성" 을 loud 표시하고
  ...
  origin/main"). 그때부터 drift 감지가 작동한다(자동 추론 없음·사용자 결정).
...
- **슬롯 base rebase** — 슬롯을 기록된 base(또는 `--onto <branch>`) 최신으로 rebase 한다("이 슬롯
  ...
  단일**(`<slot> --task <이름>`). 자동 rebase 없음 — **원할 때만**.
...
- **0단계 main-참조 해소** — 부트스트랩 0단계가 "슬롯이 보호 브랜치를 직접 checkout / 보호 브랜치
  원격을 origin-추적" 이라며 진입을 거부하면, 그 안내가 제시하는 `switch <slot> <branch>` 로
  작업 브랜치로 빠져나온다("이 슬롯 작업 브랜치로 전환해"). 전환과 장부 스냅 재기록이 **한 호출에**
  일어나 재진입이 diverged 로 다시 막히지 않는다.
...
지칭한다. **`rebase` 의 `--task` 는 소유 명의 축도 겸한다** — `rebase <slot> --task <이름>` 은
...
하네스 안 프로세스라 엔진이 못 본다(기계 신호 부재) — rebase 가 그 슬롯의 working
...
- `dev`: 그 submodule 을 on-branch(dev 역할)로 만든다 — 이후 브랜치 전환·부트스트랩의 selective
  resync 가 그 submodule 을 **skip**(detached pin 으로 안 낚아챈다).
- `sync`: pool selective 재동기를 수동 트리거한다. **dev(on-branch)·dirty submodule 은
...
- `set-base`: `<slot>` 은 **위치인자**(예 `work/proj_1` 또는 접두 생략 `proj_1`) — dev/sync 의
  ...
  ). **해소 불가 ref(오타·미fetch)는 rc 1 로 거부**된다 — slot HEAD 로 조용히 폴백해
...
- `rebase`: `<slot>`(단일)·`--task <이름>`(그 task 보유 전 슬롯 일괄)·**`<slot> --task <이름>`**(그 task
  ...
  - ⚠ **명의는 호출자가 밝히는 것**(자칭) — `--task <남의 task>` 를 주면 통과한다. `release --task`
    의 소유검사·세션 축(`PM_SESSION_NAME`)과 **같은 모델**이다. 이 가드는 **오조작 방지**이지
  ...
  - **자동 rebase 없음** — 원할 때만(엔진이 알아서 막 하지 않는다).
- `refresh`: **readonly 공유 슬롯 전용** — fetch → detached HEAD 를 기준(`--onto <branch>` 또는
  ...
  거부한다(문서 검증 기준면·무소유 공유 자산·갱신은 refresh 만).
- `record`: `<slot>` 위치인자 — 슬롯 lease 의 **도착 스냅**(`lease.git` 의 branch·head·recorded_at)을
  ...
  엔진이 자동 재기록하지 않는다). 장부에 없는 슬롯·스냅 불가(슬롯 경로 부재 등)는 rc 1 로
...
- 슬롯 기준점(base) = **rebase 로만 바뀌는 기대 축**(live 표시와 별개). **미기록이면 추론
  금지·사용자 질의**(엔진=상태 surface·PM=확인·사용자=결정) — `set-base` 로 명시 지정한다.
...
- readonly 공유 슬롯 = **detached·배타 대여 없음·session/pid 없음**. git 이 같은
...
- readonly 슬롯 생성(`/pm-env worktree add <repo> --readonly`)은 디스크=코드 전체 사본이라 
...
- (명령어化 4요소·청중 라벨) · (worktree/submodule lifecycle·live-HEAD) ·
  (git=진실·branch 비권위) · (슬롯 git 진실·기대 축·기준점 미기록=사용자 질의) ·
  (슬롯 git 조작 + readonly 공유 슬롯).
