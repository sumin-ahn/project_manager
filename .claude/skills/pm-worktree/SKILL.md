...
해소하나, 미해소(0개)·모호(≥2)면 rc 1 로 명확히 실패한다(침묵 no-op 아님). `set-base`/`status` 는
대상 슬롯을 **위치인자 `<slot>`** 으로 직접 지정한다(임의 슬롯 pool 관리 — 자기 세션 슬롯이 아닐 수
있음). submodule 경로·슬롯은 형식/목록 검증을 거쳐 슬롯 경계 밖(절대경로·`..`)은 거부된다.
...
- 스킬은 backbone 호출만 하는 얇은 래퍼 — `dev`/`sync`/`set-base`/`status` 의 stdout(무엇을 했는지·
  skip 사유·경고·조회 결과)을 PM 이 읽고 사용자에게 보고한다.
