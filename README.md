...
│       ├── pm_role.md            #   PM 세션 인계 문서 (운영 규칙·결정 권한·skill 카탈로그)
...
| `.project_manager/tools/pm_handoff.py` | 🟡 pm_role.md 형식 결합 | 세션 식별 표·인계 프롬프트 코드블록 앵커에 정규식이 묶여 있다. pm_role.md 의 해당 절 형식을 바꾸면 정규식도 같이 바꿔야 한다. |
...
| `pm_role.md` | ✅ 도메인 무관 | PM 운영 모델·결정 권한·핸드오프 절차·skill 카탈로그. `{{USER_GATE_ITEMS}}`·"진행 중인 의사결정"·"남은 작업 전체 그림" 만 채움. |
