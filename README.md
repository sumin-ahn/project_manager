...
설계가 필요한 주제는 **spike** 로 다룬다. 세션이 혼자 결정하지 않고, 실측한 현황과 옵션을
사용자와 한 절씩 합의하며 문서로 다듬고, 합의가 끝나면 봉인해서(sealed) 불변 기록으로 남긴다.
거기서 굳힌 결정은 **ADR**(Architecture Decision Record) 로 발행한다. ADR 은 "왜 이렇게
만들었나"의 역사라서, 시간이 지나 맥락을 잃은 세션도 결정의 근거를 문서에서 복원할 수 있다 —
compaction 으로 잃기 쉬운 것이 바로 이런 결정의 맥락이다.
...
설계 결정이 필요한 주제는 spike 로 연다. 옵션을 함께 좁혀 가고, 합의된 결정은 ADR 로 남는다:

```text
멀티 세션 정체성 문제를 설계 spike 로 다뤄줘.
```

작업하며 알게 된 도메인 지식은 domain 페이지로 채록한다. 페이지는 코드에 연결돼 있어서
(`covers:`), 코드가 바뀌면 낡은 페이지에 stale 표시가 붙는다:
