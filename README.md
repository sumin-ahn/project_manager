...
받는다.) 채택자는 모노레포 구조(`templates/`·루트 도그푸딩)를 몰라도 된다 — import 된 폴더 안에서
`.project_manager/` + 어댑터층이 그대로 자기 것이 된다.
...
#    - {{PROJECT_CONSTRAINTS}} → 진입 문서·agents/*.md·skills/pm-dev-delegate
...
| 토큰 | 어디에 | 무엇을 채우나 |
|---|---|---|
| `{{PROJECT_CONSTRAINTS}}` | 진입 문서, `agents/developer.md`, `agents/code-reviewer.md`, `skills/pm-dev-delegate` | 프로젝트의 **절대 위반 금지 제약**. 아키텍처 불변식·안전 경계 등. (예: "핵심 결정 로직 ↔ 비결정/LLM 계층 경계 분리", "외부 호출은 fail-soft") |
| `{{PROTECTED_PATHS}}` | `agents/*.md`, **`pm_role.local.md`**(overlay), `skills/pm-wave-claim` | 서브에이전트·PM 이 **건드리면 안 되는 파일/디렉토리**. (예: 운영 한도·안전 상수 config, immutable `raw/` 스냅샷) |
...
| 어댑터층 (`.claude/`·`.opencode/`) | ✅ 거의 그대로 | researcher·architect·developer·code-reviewer + PM workflow. `{{PROJECT_CONSTRAINTS}}`·`{{PROTECTED_PATHS}}` 만 채움. 세부는 각 타깃 README. |
